...
pub fn strip_redundant_demo_default_spacing(dom: &mut Dom, root: NodeId) {
    ...
    for p in dom.descendants(root, Some(&W::p())) {
        ...
        // M234: do NOT strip line=276 from pure-deleted residuals — Word keeps
        // A's demo spacing on pure-del list/body lines (paragraph_spacing_missing
        // × exported_list_font, LO ~78).
        // M305: also keep on **mixed** residuals with no pPrChange (same pair's
        // MIX "APPOINTMENT" still carries live line=276). m48/center MIX that
        // carries pPrChange (jc history) still strips.
        ...
        if line_ok && after_ok && before_ok && rule_ok {
            // M234: pure-deleted residuals keep line=276 (Word
            // paragraph_spacing_missing).
            // M305: mixed residual without pPrChange keeps line=276 too.
            // M251: when pPrChange records non-jc prior layout (tabs-only old
            // on two_col_index×tab), Word keeps live line=276.
            // M267: jc-only old (center×center_bold) strips demo line=276 —
            // Word omits live spacing; same-pipeline LO flat vs keep-live.
            // Empty-old pPrChange (M102/M232) still strips (m48 center).
            let keep_for_m251 = dom.element(ppr, &W::name("pPrChange")).is_some_and(|ch| {
                dom.element(ch, &W::p_pr()).is_some_and(|old| {
                    let kids: Vec<_> = dom
                        .elements(old, None)
                        .into_iter()
                        .filter(|&c| {
                            let n = dom.name(c);
                            n != Some(W::r_pr()) && n != Some(W::name("pPrChange"))
                        })
                        .collect();
                    if kids.is_empty() {
                        return false;
                    }
                    // Jc-only history → strip live demo spacing (Word center).
                    if kids.len() == 1 && dom.name(kids[0]) == Some(W::name("jc")) {
                        return false;
                    }
                    true
                })
            });
            // M305: MIX residual with no pPrChange (text-only change) keeps
            // demo line=276 — Word paragraph_spacing_missing×export list font.
            let keep_for_m305 = para_is_mixed_revision(dom, p)
                && dom.element(ppr, &W::name("pPrChange")).is_none();
            if !para_is_pure_deleted(dom, p) && !keep_for_m251 && !keep_for_m305 {
                to_remove.push(sp);
            }
            continue;
        }
...
/// M243c (pci_table×pre_separated_list): pure-I list body with dual pilcrow
/// `ins`+`del` on pPr/rPr (no body `delText`) — strip the del mark so Word's
/// pure-ins shape is recovered after wholesale pure-I/D residual emission.
pub fn strip_del_pilcrow_from_pure_ins_body(dom: &mut Dom, root: NodeId) {
    let del_text = W::del_text();
    for p in dom.descendants(root, Some(&W::p())) {
        // Any delText in the para → real mixed content; leave alone.
        if !dom.descendants(p, Some(&del_text)).is_empty() {
            continue;
        }
        let is_local =
            |c: NodeId, local: &str| dom.name(c).is_some_and(|nm| nm.local_name() == local);
        let kids = dom.elements(p, None);
        let has_body_ins = kids.iter().copied().any(|c| is_local(c, "ins"));
        if !has_body_ins {
            continue;
        }
        let body_del = kids.iter().copied().any(|c| is_local(c, "del"));
        if body_del {
            continue;
        }
        let Some(ppr) = kids.iter().copied().find(|&c| is_local(c, "pPr")) else {
            continue;
        };
        let Some(rpr) = dom
            .elements(ppr, None)
            .into_iter()
            .find(|&c| is_local(c, "rPr"))
        else {
            continue;
        };
        // Strip every del child under paraRPr (dual pilcrow).
        let dmarks: Vec<NodeId> = dom
            .elements(rpr, None)
            .into_iter()
            .filter(|&c| is_local(c, "del"))
            .collect();
        for dmark in dmarks {
            dom.remove(dmark);
        }
...
pub fn fold_whitespace_pure_ins_into_following_pure_del(dom: &mut Dom, root: NodeId) {
    ...
    loop {
        ...
        for i in 0..kids.len().saturating_sub(1) {
            ...
            // M238 (multipara_cell×missing_separator): empty pure-I often has
            // spacing-only pPr while pure-D title keeps `w:pStyle` (Heading1).
            // Word's MIX parks the deleted title's style + del mark — not B's
            // empty spacing. Prefer del pPr wholesale when del has pStyle and
            // the carrier does not.
            let ins_has_pstyle = dom
                .element(ins_p, &W::p_pr())
                .is_some_and(|ip| dom.element(ip, &W::name("pStyle")).is_some());
            let del_has_pstyle = dom
                .element(del_p, &W::p_pr())
                .is_some_and(|dp| dom.element(dp, &W::name("pStyle")).is_some());
            if del_has_pstyle && !ins_has_pstyle {
...
fn para_has_list_style(dom: &Dom, p: NodeId) -> bool {
    let Some(ppr) = dom.element(p, &W::p_pr()) else {
        return false;
    };
    ...
    dom.element(ppr, &W::name("pStyle"))
        .and_then(|ps| dom.attribute(ps, &W::val()))
        .is_some_and(|v| v.contains("List") || v.contains("list"))
}
...
fn should_fold_multi_del_at_document_scale(
    ...
    // M240 (lists_sub_paragraph×longer_header_sign_area): when **both** I and D
    // runs are short (<3), skip zero-overlap **content** pure-I × pure-D
    // (Word IIDd). Require both short — OR form regressed
    // pirates×table_left_indent (~54→38) by skipping needed folds on long
    // multi-del tails. Empty pure-I still folds (multipara / M90).
    //
    // M245 (comment×complex_list): long pure-I list (inss≥4) + sole/short pure-D
    // residual (dels≤2) with zero boundary Jaccard — Word keeps pure-I "FOUR"
    // then MIX/del "My comment". Folding last list item into the comment del
    // produces one MIX. Empty pure-D still folds (M90).
    if inss.len() >= 4
        && dels.len() <= 2
        && !boundary_empty_del
    ...
    if inss.len() < 3 || dels.len() < 3 {
        // M240 (lists×sign): both short + zero-jaccard content I×D, and pure-D
        // is list-styled while pure-I is not (signature into ListParagraph).
        // Broader both-short skip regressed stamped file_94 100→53.
        if inss.len() < 3
            && dels.len() < 3
            && !boundary_empty_del
            && !para_has_no_text(dom, last_ins)
            && !should_fold_ins_del_pair(dom, last_ins, first_del)
        {
            let del_is_list = para_has_list_style(dom, first_del);
            let ins_is_list = para_has_list_style(dom, last_ins);
            if del_is_list && !ins_is_list {
                return false;
            }
        }
        // M247 (auto_page_break_2×auto_page_break): short pure-I/D runs still
        // host long pure-I body × short pure-D title ("This icon appears…" ×
        // "TITLE IN HERE"). M242 only covers multi≥3; without this, fold
        // yields MIMDD… vs Word MIIIID…. Require ≥12-word last pure-I so
        // short demo residual folds (M90 / m44) stay on the fold path.
        if !boundary_empty_del
            && !para_has_no_text(dom, last_ins)
            && !should_fold_ins_del_pair(dom, last_ins, first_del)
            && para_word_atom_count(dom, last_ins) >= 12
            && {
                let dt = para_revision_body_text(dom, first_del);
                let n_tok = body_token_set(&dt).len();
                para_word_atom_count(dom, first_del) <= 6 && n_tok <= 5
            }
        {
            return false;
        }
    ...
    // M242 (invalid_list_def×line_break): incidental shared tokens such as
    // digit "1" ("1. EXECUTIVE SUMMARY" × "1.1") must not force MIX of a long
    // pure-I body into an unrelated short pure-D title when the **boundary**
    // pair itself is unrelated (Word IIIIIIDDDDDD, not IIIIIMDDDDD).
    let boundary_related =
        !boundary_empty_del && should_fold_ins_del_pair(dom, last_ins, first_del);
    let long_body_short_title =
        !boundary_empty_del && !boundary_related && para_word_atom_count(dom, last_ins) >= 12 && {
            let dt = para_revision_body_text(dom, first_del);
            let n_tok = body_token_set(&dt).len();
            para_word_atom_count(dom, first_del) <= 6 && n_tok <= 5
        };
    if long_body_short_title {
        return false;
    }
    // M243b (pci_table×pre_separated_list): after pure-I all B + pure-D all A,
    // multi-del boundary fold must not pairwise MIX every short list line into
    // the table residual when **boundary** Jaccard is zero and both runs are
    // multi-paragraph (≥4). Word keeps IIIIIIIII DDDD… (no MIX lead).
    if !boundary_empty_del && !boundary_related && inss.len() >= 4 && dels.len() >= 4 {
        return false;
    }
    for &i in inss {
        for &d in dels {
    ...
    loop {
        ...
        if !acted {
            ...
            while j < children.len() {
                ...
                // still always folds for short pure-I runs (≤3: Small Font Demo).
                if sole_del && following_content && !should_fold_ins_del_pair(dom, last_ins, d) {
                    continue;
                }
                // M245 (comment×complex_list): long pure-I list (≥4) + trailing
                // sole pure-D comment with zero boundary Jaccard — Word keeps
                // pure-I "FOUR" then MIX/del "My comment". m44 short pure-I
                // (3) still folds trailing sole-del.
                if sole_del
                    && inss.len() >= 4
                    && !para_has_no_text(dom, last_ins)
                ...
                // M238 (multipara_cell×missing_separator): empty pure-I carrier
                // often has spacing-only structural pPr while pure-D title keeps
                // `w:pStyle` (Heading1). Prior gate kept ins spacing and dropped
                // Heading1 (~70 LO). Word parks the del title style on the MIX.
                // Only when the carrier is text-empty — content pure-I folded into
                // a styled pure-D (lists×longer_header ListParagraph) must keep
                // the inserted look, not adopt the deleted list style.
                let ins_has_pstyle = dom
                    .element(last_ins, &W::p_pr())
                    .is_some_and(|ip| dom.element(ip, &W::name("pStyle")).is_some());
                let del_has_pstyle = dom
                    .element(d, &W::p_pr())
                    .is_some_and(|dp| dom.element(dp, &W::name("pStyle")).is_some());
                let carrier_text_empty = para_has_no_text(dom, last_ins);
                let adopt_del_ppr = (del_structural && !ins_structural)
                    || (ins_jc_only && del_has_spacing)
                    || (del_has_pstyle && !ins_has_pstyle && carrier_text_empty);
...
pub fn synthesize_table_cell_margins(dom: &mut Dom, root: NodeId) {
    ...
    for tbl in tbls {
        ...
        // M296: pure-D tables (del content, no ins) — Word omits synthesized
        // tblCellMar (pirates×indent mid pure-D bordered table).
        // M299: pure-I (TI) and mixed (TM) bordered tables also omit mar/ind on
        // pirates×table_border_widths (Word TI/TM have no tblCellMar/tblInd;
        // tip was synthesizing mar10/ind10 on all 7). Skip mar/ind synthesize
        // for **any** revision-marked table so tip matches Word emit.
        let has_ins = !dom.descendants(tbl, Some(&W::ins())).is_empty();
        let has_del = !dom.descendants(tbl, Some(&W::del())).is_empty()
            || !dom.descendants(tbl, Some(&W::del_text())).is_empty();
        if has_ins || has_del {
            continue;
        }
...
// M275 pure-D table empty del-shell strip REVERTED: multipara fair −35.4
// (93.6→58.1) despite Word-shaped pirates del counts (11→7). trPr/del shells
// from mark_fully_revised_rows are LO-positive under our emit path.

/// M279 — pure-D multi-row table immediately before pure-I 1-row table: Word
/// meshes the pure-I cells into the pure-D table's first row as ins
/// (pirates×table_border_widths after M278: tip `TD[5row] TI[A1|B1]` vs Word
/// `TM[A1|B1+Flats…]`). Move pure-I cell ins runs into matching pure-D first-row
/// cells (before existing del body), drop pure-I table. Gates: pure-D table has
/// delText and no ins; pure-I table 1 row, 1–4 cols, short cell text (≤4 tok);
/// pure-D first row has ≥ pure-I col count; optional empty pure-I paras between.
/// Fair tip-to-tip; keep only if net ≥0.
pub fn mesh_following_pure_ins_table_into_pure_del_table(dom: &mut Dom, root: NodeId) {
    let Some(body) = dom.element(root, &W::body()) else {
        return;
    };
    let kids: Vec<NodeId> = dom
    ...
    let table_is_pure_d = |tbl: NodeId| {
        !dom.descendants(tbl, Some(&W::del_text())).is_empty()
            && dom.descendants(tbl, Some(&W::ins())).is_empty()
    };
    let table_is_pure_i = |tbl: NodeId| {
        !dom.descendants(tbl, Some(&W::ins())).is_empty()
            && dom.descendants(tbl, Some(&W::del())).is_empty()
            && dom.descendants(tbl, Some(&W::del_text())).is_empty()
    };
    let cell_text = |tc: NodeId| {
        let mut t = String::new();
        for tn in dom.descendants(tc, Some(&W::t())) {
    ...
    for i in 0..kids.len().saturating_sub(1) {
        let td = kids[i];
        if dom.name(td) != Some(W::tbl()) || !table_is_pure_d(td) {
            continue;
        }
        // Find following pure-I table, skipping empty pure-I paras only.
        let mut j = i + 1;
        while j < kids.len()
            && dom.name(kids[j]) == Some(W::p())
            && para_is_pure_inserted(dom, kids[j])
            && para_has_no_text(dom, kids[j])
        {
            j += 1;
        }
        if j >= kids.len() {
        ...
        let ti = kids[j];
        if dom.name(ti) != Some(W::tbl()) || !table_is_pure_i(ti) {
            continue;
        }
        let td_rows: Vec<NodeId> = dom.elements(td, Some(&W::name("tr")));
        let ti_rows: Vec<NodeId> = dom.elements(ti, Some(&W::name("tr")));
        if td_rows.len() < 2 || ti_rows.len() != 1 {
            continue;
        }
        let td_cells: Vec<NodeId> = dom.elements(td_rows[0], Some(&W::name("tc")));
        let ti_cells: Vec<NodeId> = dom.elements(ti_rows[0], Some(&W::name("tc")));
        if ti_cells.is_empty() || ti_cells.len() > 4 || ti_cells.len() > td_cells.len() {
            continue;
        }
        // Pure-I cells short text only.
        let mut ok = true;
        for &tc in &ti_cells {
            let t = cell_text(tc);
            if t.trim().is_empty() || tok_n(&t) > 4 {
                ok = false;
                break;
            }
        }
        ...
        // Mesh: move ins children from each pure-I cell's first para into
        // corresponding pure-D cell's first para (before del body).
        for (ci, &ti_tc) in ti_cells.iter().enumerate() {
            let td_tc = td_cells[ci];
            let Some(ti_p) = dom.elements(ti_tc, Some(&W::p())).into_iter().next() else {
                continue;
            };
            let Some(td_p) = dom.elements(td_tc, Some(&W::p())).into_iter().next() else {
                continue;
            };
            // Pure-D cell should not already have ins.
            if !dom.descendants(td_p, Some(&W::ins())).is_empty() {
                continue;
            }
            let ins_kids: Vec<NodeId> = dom
                .elements(ti_p, None)
                .into_iter()
                .filter(|&c| dom.name(c) == Some(W::ins()))
                .collect();
            if ins_kids.is_empty() {
                continue;
            }
            // Insert before first non-pPr child of pure-D para (del body).
            let first_body = dom
                .elements(td_p, None)
                .into_iter()
                .find(|&c| dom.name(c) != Some(W::p_pr()));
            for &ins_n in &ins_kids {
                if dom.parent(ins_n).is_none() {
                    continue;
                }
                dom.remove(ins_n);
                if let Some(fb) = first_body {
                    if dom.parent(fb).is_some() {
                        dom.add_before_self(fb, ins_n);
                    } else {
                        dom.add(td_p, ins_n);
                    }
                } else {
...
/// M268 / M284 — first body MIX that is **wholesale** leading pure-ins + pure-del
/// rest (pirates×table_left_indent: tip MIX title + del "A Simple…"; Word pure-I
/// title then pure-D captain log stream).
///
/// M268 un-gated fired on style demos (calibri/left) where Word **keeps** lead
/// MIX and following body is pure-I — full 04-22 median 85.91→76.04. **M284 gate:**
/// after the lead MIX the body residual must be a pure-D stream (≥2 pure-D
/// paras; empty pure-I spacers ok; non-empty pure-I aborts). Lead must not have
/// `numPr` (list MIX is M283 territory). Also: leading contiguous `w:ins`; rest
/// pure-del; both texts non-empty; no shared alnum token len≥3.
///
/// **M284 REVERTED after full 06-22** (pure-D-stream gate mass LO-neg).
/// **M300 re-enables** with tight title-length gates that exclude the LO-neg
/// keys: google_docs del=1 tok THREADS; clear_formatting ins=1 tok Ouch;
/// sd_1919 del=2 toks Chapter One. Pirates: ins≈5 del≈4 pure-D≥5 — fires.
/// Fair tip-to-tip; keep only if net≥0 and LO-neg keys stay MIX.
pub fn split_first_body_wholesale_mix_into_pure_id(dom: &mut Dom, root: NodeId) {
    let Some(body) = dom.element(root, &W::body()) else {
        return;
    };
    let kids: Vec<NodeId> = dom
    ...
    // List MIX lead is M283 territory.
    if dom
        .element(mix, &W::p_pr())
        .and_then(|p| dom.element(p, &W::num_pr()))
        .is_some()
    ...
    // Pure-D residual stream after lead MIX (≥5 pure-D). Style demos / short
    // docs often have fewer.
    let mut pure_d_count = 0usize;
    for &k in &kids[1..] {
        if dom.name(k) != Some(W::p()) {
            break;
        }
        if para_is_pure_deleted(dom, k) {
            pure_d_count += 1;
            continue;
        }
        if para_is_pure_inserted(dom, k) {
            if para_has_no_text(dom, k) {
                continue; // empty pure-I spacer
            }
            return; // non-empty pure-I body → style-demo path
        }
        break;
    }
    // M300: ≥5 pure-D (pirates captain log); M284 used ≥2 and over-fired.
    if pure_d_count < 5 {
        return;
    }
    ...
    let n_ins = tok_n(&ins_text);
    let n_del = tok_n(&del_text);
    // M300 title-length: exclude 1-token del (THREADS), 1-token ins (Ouch),
    // 2-token del (Chapter One), and huge body del in MIX (list×hello 43 toks).
    if !(4..=12).contains(&n_ins) || !(3..=12).contains(&n_del) {
        return;
    }
    // Unrelated wholesale: no shared alnum token of len ≥ 3.
    ...
    let ins_toks = tok(&ins_text);
    let del_toks = tok(&del_text);
    if ins_toks.iter().any(|t| del_toks.contains(t)) {
        return;
    }
    // Pure-I from leading ins; leave rest as pure-D on original mix.
    let pure_i = dom.new_element(W::p());
    if let Some(ppr) = dom.element(mix, &W::p_pr()) {
        let ppr_clone = dom.clone_subtree(ppr);
        let mut mark_rm = Vec::new();
        for n in dom.descendants(ppr_clone, None) {
            let name = dom.name(n);
            if name == Some(W::ins()) || name == Some(W::del()) {
                mark_rm.push(n);
            }
        }
        for n in mark_rm {
        ...
        if let Some(rpr) = dom.element(ppr_clone, &W::r_pr())
            && dom.elements(rpr, None).is_empty()
        {
            dom.remove(rpr);
        }
        // M301 REVERTED: strip pStyle Title from pure-I was Word-shaped but fair
        // tip-to-tip −16.9 on pirates (same as prior pirates_strip_title). Keep
        // cloned MIX pPr (incl. Title) — LO-positive under emit.
        if !dom.elements(ppr_clone, None).is_empty() {
            dom.add(pure_i, ppr_clone);
        } else {
            dom.remove(ppr_clone);
        }
    }
    for &ins_n in &leading_ins {
        if dom.parent(ins_n).is_some() {
...
// M280 REVERTED: indent×pirates structure matched Word (IIIIIIIIIDTIDDDTD)
// but fair tip-to-tip −1.0 vs 02-22 cand (59.9→58.8). Word residual order
// LO-negative under our emit/layout path.

/// M278 — free-LCS with **tables**: lead pure-I paras, then pure-I tables/body,
/// then trailing pure-D residual (+ optional pure-D table). Word parks pure-D
/// residual right after the lead pure-I paras (pirates×table_border_widths:
/// `III D* TM I TI…` vs tip `III TI… D* TD`). Move trailing pure-D block after
/// lead pure-I paragraphs. Gates: ≥2 lead pure-I paras; ≥3 trailing pure-D
/// paras; middle contains ≥1 pure-I table; no MIX tables in trailing block.
/// Fair tip-to-tip; keep only if net ≥0 (multipara must stay ~93.6).
pub fn relocate_trailing_pure_d_before_pure_ins_tables(dom: &mut Dom, root: NodeId) {
    let Some(body) = dom.element(root, &W::body()) else {
        return;
    };
    let kids: Vec<NodeId> = dom
    ...
    // Classify each body child.
    let mut kinds: Vec<&'static str> = Vec::with_capacity(kids.len());
    for &k in &kids {
        let name = dom.name(k);
        if name == Some(W::tbl()) {
            let has_ins = !dom.descendants(k, Some(&W::ins())).is_empty();
            let has_del = !dom.descendants(k, Some(&W::del())).is_empty()
                || !dom.descendants(k, Some(&W::del_text())).is_empty();
            kinds.push(match (has_ins, has_del) {
                (true, true) => "TM",
                (true, false) => "TI",
                (false, true) => "TD",
                _ => "T=",
        ...
        kinds.push(match (pure_i, pure_d) {
            (true, false) => "I",
            (false, true) => "D",
            (false, false) => "E",
            (true, true) => "M",
    ...
    // Lead: pure-I paragraphs only (no tables).
    let mut lead_end = 0;
    while lead_end < kinds.len() && kinds[lead_end] == "I" {
        lead_end += 1;
    }
    if !(2..=8).contains(&lead_end) {
        return;
    }
    // Trailing pure-D paras (+ optional pure-D table) then optional E.
    let mut trail_start = kinds.len();
    while trail_start > lead_end {
        let k = kinds[trail_start - 1];
        if k == "E" {
            trail_start -= 1;
            continue;
        }
    ...
    let mut d_end = trail_start;
    // optional trailing pure-D table
    if d_end > lead_end && kinds[d_end - 1] == "TD" {
        d_end -= 1;
    }
    let mut d_start = d_end;
    while d_start > lead_end && kinds[d_start - 1] == "D" {
        d_start -= 1;
    }
    if d_end - d_start < 3 {
        return; // need substantial pure-D residual
    }
    // Middle: between lead and pure-D must have pure-I tables and no pure-D.
    if d_start <= lead_end {
        return;
    }
    let mid = &kinds[lead_end..d_start];
    if mid
        .iter()
        .any(|&k| k == "D" || k == "TD" || k == "TM" || k == "M")
    {
        return;
    }
    let ti_count = mid.iter().filter(|&&k| k == "TI").count();
    if ti_count < 2 {
        // M278b — full 06-22: single pure-I table mid (marketing_agenda,
        // anchor_images×annot2, pre_separated×diff_before7) had TI-then-D free-LCS
        // LO~100; relocating D before TI cratered to ~50. Pirates×border has many
        // pure-I tables in mid — keep those.
        return;
    }
    // Move pure-D block (paras + optional TD table) after lead.
    let d_nodes: Vec<NodeId> = kids[d_start..trail_start].to_vec();
    let anchor = kids[lead_end - 1];
    for &n in &d_nodes {
        if dom.parent(n).is_none() {
            continue;
        }
        dom.remove(n);
    }
...
// M295 I+ D+ I+ → I+ I+ D+ (doc_with_spacing By: Signature before pure-D):
// structure=Word but fair tip-to-tip −2.0 on target — REVERTED (Word shape
// LO-neg under emit). Leave free-LCS trailing pure-I after long pure-D.

/// M289 — `I+ D+ TI I* E*`: pure-D base residual sits between lead pure-I and a
/// pure-I table block. Word parks pure-I (incl. table) first, pure-D last
/// (list_with_break_from_word×list_with_table_break: ONE/A then TI then d/TWO
/// then base residual). Tip free-LCS leaves pure-D before TI. Move pure-D after
/// the pure-I table block. Gate: pure-D is immediately followed by pure-I table
/// (`TI`), not pure-D table (`TD` — pirates keeps early pure-D before TD). Fair
/// tip-to-tip; keep only if net≥0 + multipara.
pub fn relocate_mid_pure_d_after_pure_ins_table_block(dom: &mut Dom, root: NodeId) {
    let Some(body) = dom.element(root, &W::body()) else {
        return;
    };
    let kids: Vec<NodeId> = dom
    ...
    // Lead pure-I (need ≥1 non-empty content pure-I later).
    let mut i_end = 0;
    ...
    // Immediately after pure-D: pure-I table.
    if d_end >= kinds.len() || kinds[d_end] != "TI" {
        return;
    }
    // Rest after pure-D: TI + I* only (optional trailing E stripped).
    let mut rest_end = kinds.len();
    while rest_end > d_end && kinds[rest_end - 1] == "E" {
        rest_end -= 1;
    }
    if rest_end <= d_end {
        return;
    }
    let rest = &kinds[d_end..rest_end];
    if rest.iter().any(|&k| k != "TI" && k != "I") {
        return;
    }
    // Need at least one pure-I after the table (d/TWO content) so this is a
    // genuine "next doc continues after table" block, not a lone table.
    if !rest.contains(&"I") {
        return;
    }
    // Lead must have at least one non-empty pure-I.
    let has_lead_content = kids[..i_end].iter().any(|&k| !para_has_no_text(dom, k));
    if !has_lead_content {
        return;
    }
    // Move pure-D nodes to after the pure-I table block (before trailing E).
    let d_nodes: Vec<NodeId> = kids[i_end..d_end].to_vec();
    let anchor = kids[rest_end - 1];
    for &n in &d_nodes {
        if dom.parent(n).is_none() {
            continue;
        }
...
/// M270 — wholesale pure-I* then pure-D* (no mid MIX/tables): when pure-D has
/// **no drawings** and pure-I continues after a lead title(+empties), Word often
/// parks the pure-D residual right after the lead (green_underline×header_no_rels:
/// pure-D green list before mid pure-I "Second page"). Tip free-LCS emits all
/// pure-I first. Move the pure-D block after lead pure-I+empties. Fair +17.6.
/// Abort if pure-D contains drawings (shape early-move LO-neg −21).
/// M270b: abort if any pure-D has >16 alnum tokens — long body pure-D docs
/// (listwithspacernodes×list_with_indents) already match Word as I* D*; early
/// pure-D move left trailing pure-I and LO-hurt.
pub fn relocate_wholesale_pure_d_after_lead_pure_ins(dom: &mut Dom, root: NodeId) {
    let Some(body) = dom.element(root, &W::body()) else {
        return;
    };
    let kids: Vec<NodeId> = dom
    ...
    // Only paragraphs — tables abort (multi-stream docs).
    if kids.iter().any(|&k| dom.name(k) == Some(W::tbl())) {
        return;
    }
    // Classify I/D/E only (no MIX).
    ...
    for &k in &kids {
        ...
        let pure_i = para_is_pure_inserted(dom, k);
        let pure_d = para_is_pure_deleted(dom, k);
        let kind = match (pure_i, pure_d) {
            (true, false) => 'I',
            (false, true) => 'D',
            (false, false) => 'E',
            (true, true) => return, // MIX — not wholesale I*D*
        };
        kinds.push(kind);
    }
    // Pattern: I+ D+ E*  (optional trailing equal/empty only)
    let mut i = 0;
    while i < kinds.len() && kinds[i] == 'I' {
        i += 1;
    }
    let i_end = i;
    if i_end < 2 {
        return; // need lead + something to reorder past
    }
    while i < kinds.len() && kinds[i] == 'D' {
        i += 1;
    }
    let d_end = i;
    if d_end <= i_end {
    ...
    while i < kinds.len() && (kinds[i] == 'E' || kinds[i] == 'I') {
        // allow only E after pure-D for strict I*D*E*; if more I after D, still OK
        // but our pattern is I* then D* with no I after D in kinds as classified
        // above kinds after d_end should not be I if we required pure I*D*
        i += 1;
    }
    // Strict wholesale: only I* then D* then optional E* (no I after D).
    if kinds[i_end..d_end].iter().any(|&c| c != 'D') {
        return;
    }
    if kinds[d_end..]
        .iter()
        .any(|&c| c == 'I' || c == 'D' || c == 'M')
    {
        // trailing must be empty/equal only
        if kinds[d_end..].iter().any(|&c| c != 'E') {
            return;
        }
    }
    // Need remaining pure-I after lead (otherwise already early pure-D).
    // Lead = first non-empty pure-I + following empty pure-I.
    let mut first_ne = None;
    for (j, &kid) in kids.iter().enumerate().take(i_end) {
        if !para_has_no_text(dom, kid) {
            first_ne = Some(j);
            break;
        }
    }
    let Some(fn_i) = first_ne else {
        return;
    };
    let mut lead_end = fn_i + 1;
    while lead_end < i_end && para_has_no_text(dom, kids[lead_end]) {
        lead_end += 1;
    }
    // Remaining pure-I after lead that would move past pure-D.
    if lead_end >= i_end {
        return; // pure-D already immediately after lead
    }
    // Pure-D must not contain drawings (shape LO-neg).
    let drawing = W::name("drawing");
    for &d in &kids[i_end..d_end] {
        if !dom.descendants(d, Some(&drawing)).is_empty()
            || !dom.descendants(d, Some(&W::name("pict"))).is_empty()
        {
            return;
        }
    }
    // M270b — only short pure-D residuals (list/title-like). Long body pure-D
    // (listwithspacernodes×list_with_indents, max ~79 tokens) is already
    // Word-ordered as I* D*; early pure-D move creates trailing pure-I and
    // LO-hurts. Green underline pure-D max ~10 tokens — keep those.
    let tok_n = |s: &str| {
        s.split(|c: char| !c.is_alphanumeric())
            .filter(|t| !t.is_empty())
            .count()
    ...
    for &d in &kids[i_end..d_end] {
        let t = para_revision_body_text(dom, d);
        if tok_n(&t) > 16 {
            return;
        }
    }
    // M270c — full 06-22: single short pure-D at end of long pure-I stream
    // (diff_after9 "This is the end.", missing_sectpr) was relocated early and
    // cratered LO ~100→50. Green needs ≥2 non-empty pure-D. List pure-D (numPr)
    // and wholesale I* with many remaining pure-I after lead are also LO-neg.
    let mut non_empty_d = 0usize;
    for &d in &kids[i_end..d_end] {
        if !para_has_no_text(dom, d) {
            non_empty_d += 1;
        }
        if dom
            .element(d, &W::p_pr())
    ...
    let mut remaining_ne_i = 0usize;
    for &k in &kids[lead_end..i_end] {
        if !para_has_no_text(dom, k) {
            remaining_ne_i += 1;
        }
    }
    ...
    // Move pure-D nodes to after lead (before remaining pure-I).
    let d_nodes: Vec<NodeId> = kids[i_end..d_end].to_vec();
    let anchor = kids[lead_end - 1];
    for &n in &d_nodes {
        if dom.parent(n).is_none() {
...
/// M282 / M282b — short title pure-D residual onto last pure-I list item
/// (exported_list_font×exporttest: pure-I `a`…`b`|del `APPOINTMENT`).
///
/// Original path: M270 parks pure-D mid-list → M282 folds onto last pure-I
/// **after** pure-D. **M282b (after M270c):** free-LCS often leaves pure-D at
/// the **end** of the pure-I list (`I* D`) because M270c requires ≥2 pure-D and
/// caps remaining pure-I — single-token APPOINTMENT never relocates early.
/// Fold trailing short pure-D onto the last pure-I+numPr **before** pure-D.
///
/// Gates: no tables; short pure-D (1..=3 alnum toks) with title-like alpha
/// (len ≥ 4); no drawings; either (mid-list: ≥1 pure-I+numPr before and after)
/// or (trailing: ≥2 pure-I+numPr before, none after). Fair tip-to-tip; keep if net≥0.
pub fn fold_interleaved_short_pure_d_into_last_pure_ins_list(dom: &mut Dom, root: NodeId) {
    let Some(body) = dom.element(root, &W::body()) else {
        return;
    };
    let kids: Vec<NodeId> = dom
    ...
    let has_numpr = |p: NodeId| {
        dom.element(p, &W::p_pr())
            .and_then(|pp| dom.element(pp, &W::num_pr()))
            .is_some()
    };
    let tok_n = |s: &str| {
        s.split(|c: char| !c.is_alphanumeric())
    ...
    let has_title_alpha = |s: &str| {
        s.split(|c: char| !c.is_ascii_alphabetic())
            .any(|t| t.chars().count() >= 4)
    };
    let drawing = W::name("drawing");
    let is_list_i = |k: NodeId| {
        dom.name(k) == Some(W::p())
            && para_is_pure_inserted(dom, k)
            && has_numpr(k)
            && !para_has_no_text(dom, k)
    };

    for di in 0..kids.len() {
        let d = kids[di];
        if dom.name(d) != Some(W::p()) || !para_is_pure_deleted(dom, d) {
            continue;
        }
        let del_text = para_revision_body_text(dom, d);
        if del_text.trim().is_empty() {
            continue;
        }
        let dn = tok_n(&del_text);
        if !(1..=3).contains(&dn) || !has_title_alpha(&del_text) {
            continue;
        }
        if !dom.descendants(d, Some(&drawing)).is_empty()
            || !dom.descendants(d, Some(&W::name("pict"))).is_empty()
        ...
        let mut before_list_count = 0usize;
        let mut last_before: Option<NodeId> = None;
        for &k in &kids[..di] {
            if is_list_i(k) {
                before_list_count += 1;
                last_before = Some(k);
        ...
        let mut last_after: Option<NodeId> = None;
        let mut after_list_count = 0usize;
        for &k in &kids[di + 1..] {
            if is_list_i(k) {
                after_list_count += 1;
                last_after = Some(k);
        ...
        // Mid-list (M282): target last pure-I+numPr after pure-D.
        // Trailing (M282b): no list pure-I after; need ≥2 list pure-I before;
        // target last pure-I+numPr before pure-D.
        let target = if after_list_count > 0 {
            last_after
        } else if before_list_count >= 2 {
            last_before
        } else {
            None
        };
        ...
        let del_kids: Vec<NodeId> = dom
            .elements(d, None)
            .into_iter()
            .filter(|&c| dom.name(c) == Some(W::del()))
            .collect();
        if del_kids.is_empty() {
            continue;
        }

        let mut prev = match dom.elements(target, None).last().copied() {
            Some(last) => last,
            None => match dom.element(target, &W::p_pr()) {
                Some(p) => p,
                None => {
                    let p = dom.new_element(W::p_pr());
                    dom.add(target, p);
        ...
        for &dk in &del_kids {
            if dom.parent(dk).is_none() {
                continue;
            }
            dom.remove(dk);
            dom.add_after_self(prev, dk);
            prev = dk;
        }

        // Drop emptied pure-D (pPr only or empty).
        let remain: Vec<NodeId> = dom
            .elements(d, None)
            .into_iter()
        ...
        // One junction per pass.
        break;
    }
}

/// M302 REVERTED — italic×base_ordered Word peels Three|OOXML MIX to pure-I|D
/// but fair tip-to-tip −7.07 (51.0→43.9). Free-LCS MIX is LO-positive under emit.
pub fn peel_list_label_mix_long_body_del_onto_following_pure_d(_dom: &mut Dom, _root: NodeId) {
    // Intentionally no-op (M302 LO-neg under emit).
}

/// M303 REVERTED — two_col×multi Page1 peel structure=Word but fair tip-to-tip
/// −2.33 (74.49→72.16). Free-LCS MIX LO-positive under emit; Word's own redline
/// scores higher only because Word emit differs from our pure-I|D emit.
pub fn peel_lead_list_mix_short_page_del_onto_pure_d_stream(_dom: &mut Dom, _root: NodeId) {
    // Intentionally no-op (M303 LO-neg under emit).
}

/// M292 — auto_page_break×list_enter: tip free-LCS meshes mid-length pure-I
/// heading (`Heading. Body copy for repro`, no numPr) with long base residual
/// as MIX before pure-D stream. Word peels to pure-I + pure-D. M290 only covers
/// ins ≤2 chars (list_spacer1 `b`). Gates: no tables; MIX no numPr; ins 3..=8
/// alnum toks; del ≥8 toks; following pure-D; lead pure-I. Fair tip-to-tip;
/// keep only if net≥0 (M269 Grapes has numPr — safe).
pub fn peel_mid_heading_mix_long_del_onto_following_pure_d(dom: &mut Dom, root: NodeId) {
    let Some(body) = dom.element(root, &W::body()) else {
        return;
    };
    let kids: Vec<NodeId> = dom
    ...
    for i in 0..kids.len().saturating_sub(1) {
        let mix = kids[i];
        let nxt = kids[i + 1];
        if dom.name(mix) != Some(W::p()) || dom.name(nxt) != Some(W::p()) {
            continue;
        }
        if !para_is_mixed_revision(dom, mix) {
            continue;
        }
        if !para_is_pure_deleted(dom, nxt) {
            continue;
        }
        if dom
            .element(mix, &W::p_pr())
            ...
            .is_some()
        {
            continue; // list MIX — M269 owns Grapes|body
        }
        let mut ins_only = String::new();
        for n in dom.descendants(mix, Some(&W::ins())) {
            for t in dom.descendants(n, Some(&W::t())) {
                ins_only.push_str(&dom.value_str(t));
            }
        }
        let mut del_only = String::new();
        for n in dom.descendants(mix, Some(&W::del())) {
            for t in dom.descendants(n, Some(&W::del_text())) {
                del_only.push_str(&dom.value_str(t));
            }
        }
        let n_ins = tok_n(&ins_only);
        let n_del = tok_n(&del_only);
        if !(3..=8).contains(&n_ins) {
            continue;
        }
        // M292b — full 06-22: style demos (calibri×center, file_94) mid MIX has
        // short body del (~9 toks) and Word keeps MIX (LO~100). auto_listenter del
        // is hundreds of toks. Require substantial pure-D residual (n_del ≥ 20).
        if n_del < 20 {
            continue;
        }
        let lead_i = kids[..i].iter().any(|&k| {
            dom.name(k) == Some(W::p())
                && para_is_pure_inserted(dom, k)
                && !para_has_no_text(dom, k)
        });
        ...
        let pure_d = dom.new_element(W::p());
        for &d in &del_kids {
        ...
        if dom.parent(mix).is_some() {
            dom.add_after_self(mix, pure_d);
        } else {
            dom.add(body, pure_d);
        }
        break;
...
/// M290 / M293 / M294 — list_spacer1×list_with_break_exported: tip free-LCS meshes
/// short pure-I `b` with long base residual `14.9Entire Agreement…` as MIX, then
/// pure-D for remaining base. Word peels the long del into its own pure-D
/// after short `b`. Peel all `w:del` from mid MIX (short ins ≤2 chars, long del
/// by toks or alnum length) into a new pure-D inserted after MIX when the next
/// body child is pure-D. MIX becomes pure-I.
/// M293: tables allowed **after** the MIX|pure-D junction only (bookmark×broken).
/// M294: del `Introduction1` is one alnum token — accept del with ≥10 alnum chars
/// even when n_del < 4 (pageref×restart_numbering). Fair tip-to-tip; keep if net≥0.
pub fn peel_mid_short_ins_mix_long_del_onto_following_pure_d(dom: &mut Dom, root: NodeId) {
    let Some(body) = dom.element(root, &W::body()) else {
        return;
    };
    let kids: Vec<NodeId> = dom
    ...
    for i in 0..kids.len().saturating_sub(1) {
        ...
        // M293: no table at or before the junction (bookmark residual is para-only
        // before later pure-D table). Tables later in the body are fine.
        if kids[..=i + 1]
            .iter()
            .any(|&k| dom.name(k) == Some(W::tbl()))
        {
            continue;
        }
        if !para_is_mixed_revision(dom, mix) {
        ...
        // Extract ins-only / del-only text for token gates (mixed body text
        // would over-count).
        let mut ins_only = String::new();
        for n in dom.descendants(mix, Some(&W::ins())) {
            for t in dom.descendants(n, Some(&W::t())) {
        ...
        // Short list/export labels only (`b`, `a`, `C`) — not M269 Grapes
        // (6+ letters).
        let ins_trim = ins_only.trim();
        if ins_trim.chars().count() > 2 {
            continue;
        }
        // M294: del may be one alnum token (`Introduction1`) — accept if
        // n_del ≥ 4 **or** alnum char count ≥ 10 (pageref×restart list residual).
        let d_alnum = del_only.chars().filter(|c| c.is_alphanumeric()).count();
        if n_del < 4 && d_alnum < 10 {
            continue;
        }
        // M293: allow list numPr on short labels (`a`/`b`/`C`) — bookmark×broken
        // residual list item carries numPr. M269 Grapes is blocked by ins
        // chars > 2 above (6 letters), not by numPr.
        // Need some pure-I lead before MIX (a/test before b).
        let lead_i = kids[..i].iter().any(|&k| {
            dom.name(k) == Some(W::p())
                && para_is_pure_inserted(dom, k)
                && !para_has_no_text(dom, k)
        ...
        // Insert pure-D after MIX (before following pure-D stream).
        if dom.parent(mix).is_some() {
            dom.add_after_self(mix, pure_d);
        } else {
            dom.add(body, pure_d);
...
/// M298 REVERTED — basic_list×list_enter Word peels MIX heading|`List item 1`
/// to pure-I|pure-D, but fair tip-to-tip LO −14.4 (82.9→68.5). Free-LCS MIX is
/// LO-positive under emit. Leave no-op; do not re-enable without same-pipeline
/// fair net≥0.
pub fn peel_list_numpr_mix_unrelated_del_onto_following_pure_d(_dom: &mut Dom, _root: NodeId) {
    // Intentionally no-op (M298 LO-neg under emit).
}

/// M283 — broken_list_missing×multiple_nodes: Word pure-I list prefix then
/// pure-D residual stream starting with `Item 1…`. Tip free-LCS meshes first A
/// list residual onto lead pure-I as MIX. Peel all `w:del` from lead MIX+numPr
/// into a new pure-D (with numPr) inserted at the head of the following pure-D
/// stream (after pure-I prefix). Lead becomes pure-I.
/// Gates: first non-empty is MIX+numPr; ins ≤6 alnum tokens; del non-empty;
/// ≥1 pure-I+numPr non-empty after lead before pure-D stream; pure-D stream
/// exists; no tables. Style demos lack pure-I+numPr after lead MIX (M268
/// lesson) — do not fire. Fair tip-to-tip; keep only if net ≥0 + multipara.
pub fn peel_lead_list_mix_del_onto_pure_d_stream_head(dom: &mut Dom, root: NodeId) {
    let Some(body) = dom.element(root, &W::body()) else {
        return;
    };
    let kids: Vec<NodeId> = dom
    ...
    for (i, &k) in kids.iter().enumerate() {
        ...
        let has_ins = !dom.descendants(k, Some(&W::ins())).is_empty();
        let has_del = !dom.descendants(k, Some(&W::del())).is_empty();
        let empty = para_has_no_text(dom, k) && para_revision_body_text(dom, k).trim().is_empty();
        if empty && pure_i {
            continue;
        }
        // MIX = both ins and del present (not pure).
        if has_ins && has_del && !pure_i && !pure_d {
            lead_i = Some(i);
            break;
        }
        // Non-empty non-MIX first content — not our case.
        return;
    }
    let Some(li) = lead_i else {
        return;
    };
    let mix = kids[li];
    if !has_numpr(mix) {
    ...
    let ins_text = {
        let mut t = String::new();
        for ins in dom.elements(mix, Some(&W::ins())) {
            for tn in dom.descendants(ins, Some(&W::t())) {
                t.push_str(&dom.value_str(tn));
            }
        }
        t
    ...
    let del_text = {
        let mut t = String::new();
        for d in dom.elements(mix, Some(&W::del())) {
            for tn in dom.descendants(d, Some(&W::del_text())) {
                t.push_str(&dom.value_str(tn));
            }
        }
        t
    };
    if ins_text.trim().is_empty() || del_text.trim().is_empty() {
        return;
    }
    let n_ins = tok_n(&ins_text);
    if n_ins == 0 || n_ins > 6 {
    ...
    // List residual del: at least one alnum token; prefer multi-token list
    // body (Item 1 / Text 1) — single short title del is style-demo risk.
    let n_del = tok_n(&del_text);
    if n_del < 2 {
        return;
    }
    // After lead: pure-I* (with ≥1 non-empty numPr) then pure-D stream.
    // M291 tried skipping empty dual-pilcrow / treating rPr-del content as
    // pure-I so broken×broken peels — structure closer to Word but fair
    // tip-to-tip −0.97 on broken_broken and −1.20 on broken_multi (M283
    // target). REVERTED: keep strict pure_i scan only.
    let mut j = li + 1;
    let mut saw_list_i = false;
    while j < kids.len() {
        let k = kids[j];
        if dom.name(k) != Some(W::p()) {
            return;
        ...
        if para_is_pure_inserted(dom, k) {
            if !para_has_no_text(dom, k) && has_numpr(k) {
                saw_list_i = true;
            }
            j += 1;
            continue;
    ...
    if !saw_list_i {
        return; // style demos: body pure-I without numPr, or no pure-I prefix
    }
    if j >= kids.len() {
        return;
    }
    // Must start pure-D stream.
    if !para_is_pure_deleted(dom, kids[j]) {
        return;
    }
    let pure_d_head = kids[j];
    ...
    // New pure-D para: optional numPr clone from mix, then del children.
    let pure_d = dom.new_element(W::p());
    if let Some(mppr) = dom.element(mix, &W::p_pr())
        && let Some(num) = dom.element(mppr, &W::num_pr())
    {
        let ppr = dom.new_element(W::p_pr());
        let num_clone = dom.clone_subtree(num);
        dom.add(ppr, num_clone);
        dom.add(pure_d, ppr);
    }
    for &d in &del_kids {
        if dom.parent(d).is_none() {
    ...
    // Insert pure-D at stream head (before first pure-D).
    if dom.parent(pure_d_head).is_some() {
        dom.add_before_self(pure_d_head, pure_d);
    } else {
        dom.add(body, pure_d);
    }
    // mix is now pure-I (ins only).
}

/// M271 — after M269 leaves pure-I* + MIX + pure-D* (list wholesale), Word
/// often meshes the **first** B list item with the first A residual title
/// (custom_list_numbering×custom_list1: `Num 1`|`SECTION` at lead). Relocate
/// del children of the trailing MIX onto the first pure-I; former MIX becomes
/// pure-I. Fair tip-to-tip +3.65. Gates: I+ M D+ body paras only; both ends
/// short list-ish text; MIX and first I carry numPr.
pub fn relocate_trailing_mix_del_onto_first_pure_ins_list(dom: &mut Dom, root: NodeId) {
    let Some(body) = dom.element(root, &W::body()) else {
        return;
    };
    let kids: Vec<NodeId> = dom
    ...
    // Find pure-I+ then MIX then pure-D+
    let mut i = 0;
    while i < kids.len() && dom.name(kids[i]) == Some(W::p()) && para_is_pure_inserted(dom, kids[i])
    {
        i += 1;
    }
    let i_end = i;
    if i_end < 1 || i_end >= kids.len() {
        return;
    }
    let mix = kids[i_end];
    if dom.name(mix) != Some(W::p()) {
    ...
    // Following pure-D only (at least one).
    let mut j = i_end + 1;
    while j < kids.len() && dom.name(kids[j]) == Some(W::p()) && para_is_pure_deleted(dom, kids[j])
    {
        j += 1;
    }
    if j <= i_end + 1 {
    ...
    // Rest after pure-D should not reintroduce pure-I (wholesale list residual).
    if kids[j..]
        .iter()
    ...
    let first = kids[0];
    if !para_is_pure_inserted(dom, first) {
        return;
    }
    // numPr on first and mix
    let first_num = dom
    ...
    let mix_num = dom
        ...
        .and_then(|p| dom.element(p, &W::num_pr()))
        .is_some();
    if !first_num || !mix_num {
        return;
    }
    let first_text = para_revision_body_text(dom, first);
    let mix_ins = {
        let mut t = String::new();
        for ins in dom.elements(mix, Some(&W::ins())) {
            for tn in dom.descendants(ins, Some(&W::t())) {
    ...
    if tok_n(&first_text) == 0
        || tok_n(&first_text) > 6
        || tok_n(&mix_ins) == 0
        || tok_n(&mix_ins) > 6
        || mix_del.trim().is_empty()
        || tok_n(&mix_del) > 12
    {
        return;
    }
    // M271b — only relocate when del residual is a multi-letter "section/title"
    // style token (len ≥ 4 alpha). Free-LCS already Word-shapes short residuals
    // parked on the last pure-I (complex_list `ONE`, list_def_mix `Num 1`);
    // custom_list SECTION must move to first pure-I. Without this gate M271
    // undoes correct I* M D* → M I* D* on those fixtures (LO-neg).
    let has_long_alpha = mix_del
        .split(|c: char| !c.is_ascii_alphabetic())
        .any(|t| t.chars().count() >= 4);
    if !has_long_alpha {
        return;
    }
    // Move del children from mix onto first pure-I (after pPr), preserving order.
    let del_kids: Vec<NodeId> = dom
        .elements(mix, None)
        .into_iter()
        .filter(|&c| dom.name(c) == Some(W::del()))
    ...
    let first_ppr = match dom.element(first, &W::p_pr()) {
        Some(p) => p,
        None => {
            let p = dom.new_element(W::p_pr());
            if let Some(hd) = dom.elements(first, None).first().copied() {
                dom.add_before_self(hd, p);
            } else {
                dom.add(first, p);
            }
            p
    ...
    let mut prev = first_ppr;
    // Append dels after existing body of first (after last child), not only after pPr —
    // Word order is ins then del on MIX. First is pure-I so body is ins; append dels after.
    if let Some(last) = dom.elements(first, None).last().copied() {
        prev = last;
    }
    for &d in &del_kids {
        if dom.parent(d).is_none() {
        ...
        dom.remove(d);
        dom.add_after_self(prev, d);
        prev = d;
    }
    // MIX should now be pure-I (ins only).
}



/// M285 — diff_after2×simple_ordered_list: free-LCS parks first A residual
/// ("This is a test.") on the **last** pure-I list item as MIX (`Item 4`); Word
/// meshes it onto the **first** pure-I+numPr of that contiguous list run
/// (`Lvl 1 – a`). Relocate del children from trailing MIX onto the first
/// pure-I+numPr of the pure-I+numPr run immediately before MIX; MIX becomes
/// pure-I. Gates: MIX has numPr + short del (1..=6 toks); run length ≥2;
/// following pure-D stream; no tables; target ins short (≤6 toks). Long body
/// residual (bullet Grapes|document) aborts via del tok cap. Fair tip-to-tip.
pub fn relocate_short_mix_del_onto_first_of_preceding_numpr_run(dom: &mut Dom, root: NodeId) {
    let Some(body) = dom.element(root, &W::body()) else {
        return;
    };
    let kids: Vec<NodeId> = dom
    ...
    for mi in 1..kids.len() {
        ...
        let has_ins = !dom.descendants(mix, Some(&W::ins())).is_empty();
        let has_del = !dom.descendants(mix, Some(&W::del())).is_empty();
        if !has_ins || !has_del || !has_numpr(mix) {
            continue;
        }
        // Following pure-D stream (wholesale residual after list).
        let mut pure_d_after = 0usize;
        for &k in &kids[mi + 1..] {
            if dom.name(k) != Some(W::p()) {
                break;
            }
        ...
        let n_del = tok_n(&mix_del);
        // Short residual only — long bullet body (M269) stays on last item.
        if !(1..=6).contains(&n_del) {
            continue;
        }

        // Contiguous pure-I+numPr run immediately before MIX.
        let mut run_start = mi;
        while run_start > 0 {
            let prev = kids[run_start - 1];
            if dom.name(prev) == Some(W::p())
                && para_is_pure_inserted(dom, prev)
                && has_numpr(prev)
                && !para_has_no_text(dom, prev)
            {
                run_start -= 1;
            } else {
                break;
        ...
        // Need ≥1 pure-I+numPr before MIX (run_start < mi) → total run ≥2 with MIX.
        if run_start >= mi {
            continue;
        }
        // Prefer first non-"Item N" label in the run (Word: Lvl 1 – a, not
        // second-list Item 1). Fall back to run_start.
        let is_simple_item = |s: &str| -> bool {
            let t = s.trim();
            let mut it = t.split_whitespace();
            match (it.next(), it.next(), it.next()) {
                (Some(a), Some(b), None) => {
                    a.eq_ignore_ascii_case("item") && b.chars().all(|c| c.is_ascii_digit())
                }
                _ => false,
            }
        };
        let mut target = kids[run_start];
        for &k in &kids[run_start..mi] {
            let t = para_revision_body_text(dom, k);
            if !is_simple_item(&t) {
                target = k;
                break;
            }
        }
        let target_text = para_revision_body_text(dom, target);
        if tok_n(&target_text) == 0 || tok_n(&target_text) > 6 {
            continue;
        }
...
/// M269 / M288 — pure-I list item (`numPr`) immediately before pure-D body:
/// Word meshes the last B list item into the first A body residual as MIX
/// (bullet Grapes|document; listwithspacernodes×list_with_indents
/// `Unless…`|long body even when pure-D carries `numPr`).
/// Tip free-LCS left pure-I then pure-D. Fold: move pure-I body children
/// (ins) onto the pure-D para, drop empty pure-I. Fair tip-to-tip.
/// Gates: pure-I has numPr; pure-D non-empty del; pure-I short (≤6 toks);
/// pure-D longer body than list item (M269b); pure-D either has **no** numPr
/// (classic M269) **or** long body residual ≥20 toks (M288 — list residual
/// pure-D with numPr; short labels like ONE stay pure-D). Last pure-I in
/// contiguous pure-I+numPr run.
pub fn fold_trailing_pure_ins_list_item_into_following_pure_del_body(dom: &mut Dom, root: NodeId) {
    let Some(body) = dom.element(root, &W::body()) else {
        return;
    };
    let kids: Vec<NodeId> = dom
    ...
    for i in 0..kids.len().saturating_sub(1) {
        ...
        let Some(ippr) = dom.element(ins_p, &W::p_pr()) else {
            continue;
        };
        if dom.element(ippr, &W::num_pr()).is_none() {
            continue;
        }
        let del_text = para_revision_body_text(dom, del_p);
        if del_text.trim().is_empty() {
        ...
        let n_tok = ins_text
            ...
            .filter(|t| !t.is_empty())
            .count();
        // M269b — pure-D must be a longer *body* residual than the list item
        // (bullet Grapes→long body). Short pure-D labels ("ONE") must stay
        // pure-D after pure-I "Export Restrictions" (list_table×spacer Word).
        let d_tok = del_text
            .split(|c: char| !c.is_alphanumeric())
            .filter(|t| !t.is_empty())
            .count();
        if n_tok == 0 || d_tok <= n_tok {
            continue;
        }
        // Following pure-D: no numPr (classic M269 short list|body), or long
        // body residual with numPr (M288 listspacer×indents: long list item
        // "Unless…" + long pure-D body with numPr). Short pure-D+numPr (ONE)
        // must not mesh (M269b export gate).
        let del_has_numpr = dom
            .element(del_p, &W::p_pr())
            .and_then(|p| dom.element(p, &W::num_pr()))
            .is_some();
        if del_has_numpr {
            ...
            // Long list body residual mesh — ins may be long list paragraphs.
        } else if n_tok > 6 {
            // Classic M269: short list label only (Grapes ≤6).
            continue;
        }
        // Last pure-I+numPr in a contiguous run (list→body junction).
        let is_last_in_run = i + 1 >= kids.len() || {
            let next = kids[i + 1];
            // next is del_p (not another pure-I numPr)
            !(dom.name(next) == Some(W::p())
                && para_is_pure_inserted(dom, next)
                && dom
                    .element(next, &W::p_pr())
                    .and_then(|p| dom.element(p, &W::num_pr()))
                    .is_some())
        };
        // Contiguous: previous pure-I+numPr or start / non-list.
        if !is_last_in_run {
            continue;
        }
        // Transfer numPr onto pure-D so the resulting MIX carries list props
        // for M230 park onto trailing pure-D.
        let num_opt = dom
            .element(ippr, &W::num_pr())
            .map(|n| dom.clone_subtree(n));
        // Move non-pPr children from pure-I onto pure-D (after pure-D pPr),
        // preserving order.
        let ins_kids: Vec<NodeId> = dom
            .elements(ins_p, None)
            .into_iter()
        ...
        let del_ppr = match dom.element(del_p, &W::p_pr()) {
            Some(p) => p,
            None => {
                let p = dom.new_element(W::p_pr());
                if let Some(first) = dom.elements(del_p, None).first().copied() {
                    dom.add_before_self(first, p);
                } else {
                    dom.add(del_p, p);
                }
        ...
        if let Some(num_clone) = num_opt
            && dom.element(del_ppr, &W::num_pr()).is_none()
        {
            if let Some(rpr) = dom.element(del_ppr, &W::r_pr()) {
                dom.add_before_self(rpr, num_clone);
            } else {
                dom.add_first(del_ppr, num_clone);
            }
        }
        let mut prev = del_ppr;
        for &c in &ins_kids {
            if dom.parent(c).is_none() {
                continue;
            }
            dom.remove(c);
            dom.add_after_self(prev, c);
            prev = c;
        }
        // Drop emptied pure-I (pPr only or empty).
        let remain: Vec<NodeId> = dom
            .elements(ins_p, None)
            .into_iter()
            .filter(|&c| dom.name(c) != Some(W::p_pr()))
        ...
        // One junction per pass; M230 may then park numPr from MIX.
        break;
    }
}

/// M277 — lead pure-I + pure-D both short + both `numPr`: Word meshes into one
/// MIX title (dropcaps×exported_list_font: `APPOINTMENT`|`Dropcaps` as Heading2
/// MIX). Tip free-LCS leaves pure-I then pure-D. Fold ins onto pure-D, drop
/// pure-I, strip `numPr` from the MIX (Word bare style / Heading2, not dual
/// list marks). Gates: first non-empty body pure-I; both ≤2 tokens; both
/// numPr; pure-D has non-empty delText. Fair tip-to-tip; keep only if net ≥0.
pub fn fold_lead_short_dual_numpr_pure_i_into_following_pure_d(dom: &mut Dom, root: NodeId) {
    let Some(body) = dom.element(root, &W::body()) else {
        return;
    };
    let kids: Vec<NodeId> = dom
    ...
    // First non-empty pure-I (skip leading empties).
    let mut i = 0;
    while i < kids.len()
        && dom.name(kids[i]) == Some(W::p())
        && para_is_pure_inserted(dom, kids[i])
        && para_has_no_text(dom, kids[i])
    {
        i += 1;
    }
    if i + 1 >= kids.len() {
    ...
    // Require lead is at document start (only empties before).
    if kids[..i].iter().any(|&k| {
        !(dom.name(k) == Some(W::p()) && para_is_pure_inserted(dom, k) && para_has_no_text(dom, k))
    }) {
        return;
    }
    let ins_p = kids[i];
    ...
    let has_num = |p: NodeId| {
        dom.element(p, &W::p_pr())
            .and_then(|pp| dom.element(pp, &W::num_pr()))
            .is_some()
    };
    if !has_num(ins_p) || !has_num(del_p) {
        return;
    }
    let tok_n = |s: &str| {
        s.split(|c: char| !c.is_alphanumeric())
    ...
    if tok_n(&ins_text) == 0
        || tok_n(&ins_text) > 2
        || del_text.trim().is_empty()
        || tok_n(&del_text) > 2
    {
        return;
    }
    // Move non-pPr children from pure-I onto pure-D (after pPr): ins then del.
    let ins_kids: Vec<NodeId> = dom
        .elements(ins_p, None)
        .into_iter()
        .filter(|&c| dom.name(c) != Some(W::p_pr()))
    ...
    // Strip numPr from pure-D (Word MIX is not dual-list).
    if let Some(num) = dom.element(del_ppr, &W::num_pr()) {
        dom.remove(num);
    }
    let mut prev = del_ppr;
    // Insert ins kids before existing del body (Word: ins then del).
    let del_body: Vec<NodeId> = dom
        .elements(del_p, None)
        .into_iter()
        .filter(|&c| dom.name(c) != Some(W::p_pr()))
        .collect();
    let first_del_body = del_body.first().copied();
    for &c in &ins_kids {
        if dom.parent(c).is_none() {
            continue;
        }
        dom.remove(c);
        if let Some(fd) = first_del_body {
            if dom.parent(fd).is_some() {
                dom.add_before_self(fd, c);
            } else {
                dom.add_after_self(prev, c);
            }
        } else {
...
/// M258 (two_col_index×tab): on the **last** content residual MIX, peel a
/// trailing pure-D whose text is a single digit after an ins into its own
/// following pure-D para. Word: residual `D space + Page + I6` then pure-D
/// `5`; tip kept `I6 + D5` on one para. Does **not** peel mid-doc I5+D7
/// (Word keeps those together on Page2 residual).
pub fn peel_trailing_digit_del_from_last_residual(dom: &mut Dom, root: NodeId) {
    let Some(body) = dom.element(root, &W::body()) else {
        return;
    };
    let kids: Vec<NodeId> = dom
        ...
        .filter(|&k| dom.name(k) != Some(W::name("sectPr")))
        .collect();
    // Last paragraph that has body text (skip trailing empty).
    ...
    // Body kids excluding pPr. Tip often ends residual with empty ins (br)
    // after digit del (`I6 D5 I(br)`); Word parks br inside the digit ins and
    // pure-D digit on the next para. Skip trailing empty ins when scanning.
    let mix_kids: Vec<NodeId> = dom.elements(mix, None);
    let is_empty_ins = |c: NodeId| -> bool {
        if dom.name(c) != Some(W::ins()) {
            return false;
        }
        let mut t = String::new();
    ...
    let mut skipped_empty_ins: Vec<NodeId> = Vec::new();
    for &c in mix_kids.iter().rev() {
        if dom.name(c) == Some(W::p_pr()) {
            continue;
        }
        if is_empty_ins(c) && trailing_del.is_empty() {
            skipped_empty_ins.push(c);
            continue;
        }
        if dom.name(c) == Some(W::del()) {
            trailing_del.push(c);
    ...
    // Immediately before trailing del (ignoring empty ins) must be content ins.
    let mut saw_ins_before = false;
    for &c in mix_kids.iter().rev() {
        if dom.name(c) == Some(W::p_pr())
            || trailing_del.contains(&c)
            || skipped_empty_ins.contains(&c)
        {
            continue;
        }
        if dom.name(c) == Some(W::ins()) && !is_empty_ins(c) {
            saw_ins_before = true;
        }
        break;
    }
    ...
    let trimmed = del_text.trim();
    // Single digit only (Word pure-D "5"); not multi-digit or words.
    if trimmed.len() != 1 || !trimmed.chars().next().is_some_and(|c| c.is_ascii_digit()) {
        return;
    }
    // Must leave other content on mix (equal Page + ins).
    ...
    // New pure-D para after mix.
    // M258b attempted: clone residual tabs/spacing/pPrChange onto pure-D "5"
    // (Word has them). Fair LO two_col 69.6→68.7 (−0.9 vs tip; −4.4 vs 19-21
    // run cand). Bare pure-D digit scores better under LO for our emit.
    let pure_d = dom.new_element(W::p());
    for &d in &trailing_del {
        if dom.parent(d).is_some() {
            dom.remove(d);
            dom.add(pure_d, d);
        }
    }
    // Insert pure_d after mix.
    if let Some(next) = kids.get(i + 1).copied() {
        if dom.parent(next).is_some() {
            dom.add_before_self(next, pure_d);
        } else {
            dom.add(body, pure_d);
        }
    } else if let Some(sect) = dom
        .elements(body, None)
        .into_iter()
        .find(|&k| dom.name(k) == Some(W::name("sectPr")))
    {
        dom.add_before_self(sect, pure_d);
    } else {
        dom.add(body, pure_d);
    }
}

/// M252 (two_col_index×tab): peel trailing whitespace-only `w:del` off a para
/// that otherwise has only equal body runs onto the **following** residual
/// para. Word keeps Page4 as clean equal and parks the residual del space on
/// the next residual (`D space + Page + I6`); tip glued the space onto Page4.
///
/// Gate: trailing del text is only whitespace; current para has no body
/// `w:ins`; next para has body del or ins (residual). Does not move
/// alphanumeric dels (other peels handle those).
pub fn peel_trailing_ws_del_onto_following_residual(dom: &mut Dom, root: NodeId) {
    let Some(body) = dom.element(root, &W::body()) else {
        return;
    };
    loop {
        ...
        for i in 0..kids.len().saturating_sub(1) {
            let cur = kids[i];
            let nxt = kids[i + 1];
            if dom.name(cur) != Some(W::p()) || dom.name(nxt) != Some(W::p()) {
                continue;
            }
            // No body ins on current (equal + trailing del only).
            let cur_body_ins = dom
                .elements(cur, None)
                .into_iter()
                .any(|c| dom.name(c) == Some(W::ins()));
            if cur_body_ins {
                continue;
            }
            let nxt_has_rev = !dom.descendants(nxt, Some(&W::del())).is_empty()
                || !dom.descendants(nxt, Some(&W::ins())).is_empty();
            if !nxt_has_rev {
                continue;
            }
            let cur_kids: Vec<NodeId> = dom.elements(cur, None);
            let mut trailing_ws_del: Vec<NodeId> = Vec::new();
            for &c in cur_kids.iter().rev() {
                if dom.name(c) == Some(W::p_pr()) {
                    continue;
                }
                if dom.name(c) == Some(W::del()) {
                    let mut t = String::new();
                    for tn in dom.descendants(c, Some(&W::del_text())) {
                        t.push_str(&dom.value_str(tn));
                    }
                    for tn in dom.descendants(c, Some(&W::t())) {
                        t.push_str(&dom.value_str(tn));
                    ...
                    trailing_ws_del.push(c);
                    continue;
                }
                break;
            }
            trailing_ws_del.reverse();
            if trailing_ws_del.is_empty() {
                continue;
            }
            // Must keep some equal body content on current.
            let has_equal_body = cur_kids.iter().any(|&c| {
                dom.name(c) == Some(W::r())
                    && dom
            ...
            let nxt_body_first = dom
                .elements(nxt, None)
                .into_iter()
                .find(|&c| dom.name(c) != Some(W::p_pr()));
            for &d in &trailing_ws_del {
                if dom.parent(d).is_none() {
                    continue;
                }
                dom.remove(d);
                if let Some(first) = nxt_body_first {
                    dom.add_before_self(first, d);
                } else {
                    dom.add(nxt, d);
                }
            }
...
/// M260 (two_col_index×tab): pure-I "Page4" keeps B's live spacing line=276 in
/// Word; tip produced tabs-only pPr (A's skeleton) with rPr/ins pilcrow and
/// equal body runs. When a pure-I-style para (no body del) has tabs but no
/// spacing, and the previous content para carries spacing, copy that spacing
/// onto the pure-I pPr so line height matches Word.
pub fn inherit_spacing_onto_pure_ins_from_prev(dom: &mut Dom, root: NodeId) {
    let Some(body) = dom.element(root, &W::body()) else {
        return;
    };
    let kids: Vec<NodeId> = dom
    ...
    for i in 1..kids.len() {
        let prev = kids[i - 1];
        let cur = kids[i];
        if dom.name(prev) != Some(W::p()) || dom.name(cur) != Some(W::p()) {
            continue;
        }
        // Current: no body del (pure-I / equal insert mark), has tabs, no spacing.
        let cur_body_dels = {
            let kids_c = dom.elements(cur, None);
            kids_c.iter().any(|&c| dom.name(c) == Some(W::del()))
        };
        if cur_body_dels {
            continue;
        }
        let Some(cur_ppr) = dom.element(cur, &W::p_pr()) else {
            continue;
        };
        if dom.element(cur_ppr, &W::name("tabs")).is_none() {
            continue;
        }
        if dom.element(cur_ppr, &W::name("spacing")).is_some() {
            continue;
        }
        // Previous: has spacing to inherit.
        let Some(prev_ppr) = dom.element(prev, &W::p_pr()) else {
            continue;
        };
        let Some(prev_sp) = dom.element(prev_ppr, &W::name("spacing")) else {
            continue;
        };
        // Prefer previous that is pure-D or mixed residual (has del somewhere).
        let prev_has_del = !dom.descendants(prev, Some(&W::del())).is_empty();
        if !prev_has_del {
            continue;
        }
        // Same tab skeleton (two_col right-dot tabs) — avoid borrowing spacing
        // across unrelated layout families.
        let tab_sig = |ppr: NodeId| -> String {
            dom.element(ppr, &W::name("tabs"))
                .map(|t| dom.serialize_element(t))
                .unwrap_or_default()
        };
        if tab_sig(prev_ppr) != tab_sig(cur_ppr) || tab_sig(cur_ppr).is_empty() {
            continue;
        }
        let sp = dom.clone_subtree(prev_sp);
        // Insert spacing after tabs when present (CT_PPr order: tabs before spacing).
        if let Some(tabs) = dom.element(cur_ppr, &W::name("tabs")) {
            dom.add_after_self(tabs, sp);
        } else {
            dom.add_first(cur_ppr, sp);
        }
    }
}

/// M261 (two_col_index×tab): strip `w:pPrChange` from the **section residual**
/// MIX paragraph (carries `w:sectPr`) when live spacing is present and the
/// change record is tabs-only (M251 spacing-addition history). Word residual
/// " Page6" keeps live tabs+spacing without pPrChange; mid-body MIX page
/// labels (Page2) and pure-D pages keep pPrChange. Gate: body del+ins, live
/// spacing, tabs-only old pPrChange, and `sectPr` on this pPr.
pub fn strip_tabs_only_pprchange_from_mix_residual(dom: &mut Dom, root: NodeId) {
    let Some(body) = dom.element(root, &W::body()) else {
        return;
    };
    let kids: Vec<NodeId> = dom
    ...
    for &p in &kids {
        ...
        // Body del + body ins (MIX residual), not pure-D / pure-I.
        let body_kids: Vec<NodeId> = dom
            .elements(p, None)
            .into_iter()
            .filter(|&c| dom.name(c) != Some(W::p_pr()))
            .collect();
        let has_del = body_kids.iter().any(|&c| dom.name(c) == Some(W::del()));
        let has_ins = body_kids.iter().any(|&c| dom.name(c) == Some(W::ins()));
        if !has_del || !has_ins {
            continue;
        }
        let Some(ppr) = dom.element(p, &W::p_pr()) else {
        ...
        // Only the trailing section residual (Word two_col residual has sectPr
        // on the MIX " Page6" para). Mid MIX page labels keep pPrChange.
        if dom.element(ppr, &W::name("sectPr")).is_none() {
            continue;
        }
        // Live spacing must already be present (M251 applied).
        if dom.element(ppr, &W::name("spacing")).is_none() {
            continue;
        }
        ...
        let Some(old_pr) = dom.element(chg, &W::p_pr()) else {
            continue;
        };
        // Old record is tabs-only (spacing addition history) — no spacing in old.
        if dom.element(old_pr, &W::name("spacing")).is_some() {
            continue;
        }
        if dom.element(old_pr, &W::name("tabs")).is_none() {
            continue;
        }
        // Reject if old has other layout props we care about (jc/ind).
        if dom.element(old_pr, &W::name("jc")).is_some()
            || dom.element(old_pr, &W::name("ind")).is_some()
        {
            continue;
        }
        dom.remove(chg);
...
/// M305 — mixed residual with **no** live spacing and **no** `pPrChange`,
/// immediately followed by a pure-D that still carries demo `line=276`: Word
/// keeps the same demo spacing live on the MIX (paragraph_spacing_missing ×
/// exported_list_font "APPOINTMENT"). Tip took B's bare pPr (no spacing). Copy
/// the following pure-D's demo spacing onto the MIX. Does not fire when MIX
/// already has pPrChange (m48/center jc history must stay bare of line=276).
pub fn ensure_demo_line276_on_mix_before_pure_del(dom: &mut Dom, root: NodeId) {
    let Some(body) = dom.element(root, &W::body()) else {
        return;
    };
    let kids: Vec<NodeId> = dom
    ...
    for i in 0..kids.len().saturating_sub(1) {
        let mixed = kids[i];
        let nxt = kids[i + 1];
        if dom.name(mixed) != Some(W::p()) || dom.name(nxt) != Some(W::p()) {
            continue;
        }
        if !para_is_mixed_revision(dom, mixed) {
            continue;
        ...
        // m48/center: pPrChange present → do not synthesize demo spacing.
        if dom.element(mppr, &W::name("pPrChange")).is_some() {
            continue;
        }
        if dom.element(mppr, &W::name("spacing")).is_some() {
            continue;
        }
        let Some(nppr) = dom.element(nxt, &W::p_pr()) else {
            continue;
        };
        let Some(nsp) = dom.element(nppr, &W::name("spacing")) else {
            continue;
        };
        let line = dom.attribute(nsp, &W::name("line")).unwrap_or("");
        let after = dom.attribute(nsp, &W::name("after")).unwrap_or("");
        let before = dom.attribute(nsp, &W::name("before")).unwrap_or("");
        let rule = dom.attribute(nsp, &W::name("lineRule")).unwrap_or("");
        // Only demo-default pattern (same as strip_redundant keep set).
        let demo = line == "276"
            && (after.is_empty() || after == "200")
            && before.is_empty()
            && (rule.is_empty() || rule == "auto");
        if !demo {
            continue;
        }
        let sp = dom.clone_subtree(nsp);
        // spacing before rPr (Word order: spacing then rPr/del).
        if let Some(rpr) = dom.element(mppr, &W::r_pr()) {
            dom.add_before_self(rpr, sp);
        } else if let Some(first) = dom.elements(mppr, None).first().copied() {
            dom.add_before_self(first, sp);
        } else {
            dom.add(mppr, sp);
        }
    }
...
/// M304 — pure-I+numPr list items in a wholesale pure-I list prefix get Word's
/// default single spacing (`line=240 lineRule=auto`) when live spacing is
/// absent (exported_list_font×exporttest pure-I `a`/`x`). Source B often omits
/// spacing; Word Compare synthesizes it. Gate: body starts pure-I (same as
/// M286); only pure-I+numPr non-empty; no existing live spacing. Fair tip-to-tip.
pub fn ensure_default_single_spacing_on_pure_ins_list_items(dom: &mut Dom, root: NodeId) {
    let Some(body) = dom.element(root, &W::body()) else {
        return;
    };
    // Gate: first non-empty body para is pure-I (wholesale B list).
    let mut starts_pure_i = false;
    for p in dom.elements(body, None) {
        ...
        starts_pure_i = has_body_ins && !has_body_del;
        break;
    }
    if !starts_pure_i {
        return;
    ...
    for p in dom.elements(body, None) {
        ...
        // M262n: pure-I ListParagraph+numPr must omit line=240 (comment×complex).
        // Do not re-add default spacing onto ListParagraph.
        if let Some(ps) = dom.element(ppr, &W::name("pStyle")) {
            let val = dom.attribute(ps, &W::val()).unwrap_or_default();
            if val == "ListParagraph" {
                continue;
            }
        }
        // Live spacing already present — leave alone.
        ...
        dom.set_attribute_value(sp, &W::name("line"), Some("240"));
        dom.set_attribute_value(sp, &W::name("lineRule"), Some("auto"));
        // CT_PPr: numPr before spacing; insert after numPr when present.
        if let Some(np) = dom.element(ppr, &W::num_pr()) {
            dom.add_after_self(np, sp);
        } else {
            dom.add(ppr, sp);
        }
    }
...
/// M286 — strip `w:pPrChange` from pure-I list items (`numPr`) only when the
/// body **starts** with a pure-I list (wholesale B list; list_table×spacer).
/// Custom-list (lead MIX then pure-I) keeps pPrChange — fair LO-neg when
/// stripped (−1.8). Primary fix also in `apply_numbering_id_remaps_to_document`.
pub fn strip_pprchange_from_pure_ins_list_items(dom: &mut Dom, root: NodeId) {
    let Some(body) = dom.element(root, &W::body()) else {
        return;
    };
    // Gate: first non-empty body para is pure-I (not MIX).
    let mut starts_pure_i = false;
    for p in dom.elements(body, None) {
        if dom.name(p) != Some(W::p()) {
            if dom.name(p) == Some(W::name("sectPr")) {
    ...
    let ppr_change = W::name("pPrChange");
    let mut to_remove = Vec::new();
    for p in dom.elements(body, None) {
        if dom.name(p) != Some(W::p()) {
            continue;
        ...
        for c in dom.elements(ppr, None) {
            if dom.name(c).as_ref() == Some(&ppr_change) {
                to_remove.push(c);
            }
        }
    }
...
/// M262n (comment×complex_list): strip body single-line spacing
/// (`after=0 line=240 lineRule=auto`) from pure-I **ListParagraph + numPr**
/// only. M262 broad pure-I strip regressed multipara (PARTIES bare pPr with
/// the same spacing pattern, fair 93.6→71.2). Word omits that spacing on
/// pure-I list items while multipara needs it for LO.
pub fn strip_single_line_spacing_from_pure_ins_listpara(dom: &mut Dom, root: NodeId) {
    let spacing_name = W::name("spacing");
    let mut to_remove = Vec::new();
    for p in dom.descendants(root, Some(&W::p())) {
        let has_body_del = dom
            .elements(p, None)
            .into_iter()
            .any(|c| dom.name(c) == Some(W::del()));
        if has_body_del {
            continue;
        }
        ...
        // ListParagraph + numPr only (not multipara PARTIES bare pPr).
        let is_list_para = dom
            .element(ppr, &W::name("pStyle"))
            .is_some_and(|st| dom.attribute(st, &W::name("val")) == Some("ListParagraph"));
        if !is_list_para || dom.element(ppr, &W::name("numPr")).is_none() {
            continue;
        }
        let Some(sp) = dom.element(ppr, &spacing_name) else {
            continue;
        ...
        let rule = dom.attribute(sp, &W::name("lineRule")).unwrap_or("");
        if line == "240"
            && (after == "0" || after.is_empty())
            && (before.is_empty() || before == "0")
            && (rule == "auto" || rule.is_empty())
        {
            to_remove.push(sp);
        }
    }
...
/// M263 (annot2×annotations_import): convert pure-D empty → pure-I empty when
/// immediately before a pure-I empty. Word after pure-I "Test 1" has two pure-I
/// empties then "Test 2"; tip had pure-D empty + pure-I empty (A empty meshed as
/// del mark). Converting (not dropping) preserves both empties as pure-I.
/// Pure-D empties Word keeps after duration sit before a table — not before
/// pure-I empty — so this gate leaves those.
pub fn drop_pure_d_empty_before_pure_i_empty(dom: &mut Dom, root: NodeId) {
    // M263b: convert pure-D empty → pure-I empty when followed by pure-I empty
    // (Word annot2: two pure-I empties after Test 1). Dropping alone left only
    // one pure-I empty (B's second empty was the pure-D mark).
    let Some(body) = dom.element(root, &W::body()) else {
        return;
    };
    let kids: Vec<NodeId> = dom
    ...
    for i in 0..kids.len().saturating_sub(1) {
        ...
        if !para_is_empty_pure_del(dom, cur) {
            continue;
        }
        if !para_is_empty_pure_ins(dom, nxt) {
            continue;
        }
        // Flip pPr/rPr/del → ins (mark-only pure-D empty becomes pure-I empty).
        let Some(ppr) = dom.element(cur, &W::p_pr()) else {
            continue;
        };
        let Some(rpr) = dom.element(ppr, &W::r_pr()) else {
            continue;
        };
        let Some(del) = dom.element(rpr, &W::del()) else {
            continue;
        };
        // Preserve id/author/date when present.
        let id = dom.attribute(del, &W::id()).map(|s| s.to_string());
        let author = dom.attribute(del, &W::author()).map(|s| s.to_string());
        let date = dom.attribute(del, &W::date()).map(|s| s.to_string());
        dom.remove(del);
...
/// Empty pure-deleted paragraph: no body text, del mark (body or pPr/rPr), no ins.
fn para_is_empty_pure_del(dom: &Dom, p: NodeId) -> bool {
    if dom.name(p) != Some(W::p()) {
        return false;
    }
    if !para_has_no_text(dom, p) {
...
/// M264 (annot2×annotations_import): Word pure-I "Enter duration" (SDT body)
/// before a table carries a **deleted** pilcrow (`pPr/rPr/w:del`). Tip keeps
/// pure-I pilcrow. Flip mark-only ins→del on pure-I **SDT** content paras
/// immediately followed by a table. Plain pure-I titles before tables
/// (pirates table indent) are exempt.
pub fn del_pilcrow_on_pure_ins_before_table(dom: &mut Dom, root: NodeId) {
    let Some(body) = dom.element(root, &W::body()) else {
        return;
    };
    let kids: Vec<NodeId> = dom
    ...
    for i in 0..kids.len().saturating_sub(1) {
        let cur = kids[i];
        let nxt = kids[i + 1];
        if dom.name(cur) != Some(W::p()) || dom.name(nxt) != Some(W::name("tbl")) {
            continue;
        }
        // Pure-I: has ins, no delText/body del.
        if !dom.descendants(cur, Some(&W::del_text())).is_empty() {
            continue;
        }
        let has_body_del = dom
            .elements(cur, None)
        ...
        let has_ins = !dom.descendants(cur, Some(&W::ins())).is_empty()
            || para_mark_revision(dom, cur, &W::ins());
        if !has_ins {
            continue;
        }
        // Must have some non-empty text (duration content), not empty pure-I.
        if para_has_no_text(dom, cur) {
            continue;
        }
        // Annot2: "Enter duration" lives in w:sdt. Do not stamp del pilcrow on
        // plain pure-I titles before tables (pirates "Table with no left indent").
        if dom.descendants(cur, Some(&W::name("sdt"))).is_empty() {
            continue;
        }
        // Flip pPr/rPr ins → del when present; else ensure del mark.
        let ppr = match dom.element(cur, &W::p_pr()) {
            Some(p) => p,
            None => {
                let p = dom.new_element(W::p_pr());
                dom.add_first(cur, p);
        ...
        if dom.element(rpr, &W::del()).is_some() {
            continue; // already del pilcrow
        }
        // Remove mark ins if present.
        if let Some(ins) = dom.element(rpr, &W::ins()) {
            let id = dom.attribute(ins, &W::id()).map(|s| s.to_string());
            let author = dom.attribute(ins, &W::author()).map(|s| s.to_string());
            let date = dom.attribute(ins, &W::date()).map(|s| s.to_string());
            dom.remove(ins);
...
/// M287 — list_table_break×listwithspacernodes: Word has empty pure-I between
/// last pure-I list item (`Export Restrictions`) and pure-D residual stream
/// (`ONE`…). Tip free-LCS goes list→pure-D; M85a strips empties before pure-D
/// for catalog demos, so insert **after** that peel. Gates: body starts pure-I;
/// pure-I+numPr non-empty immediately before pure-D; pure-D del 1..=3 tokens.
/// Fair tip-to-tip; keep only if net ≥0.
pub fn insert_empty_pure_ins_before_list_residual_pure_d(dom: &mut Dom, root: NodeId) {
    let Some(body) = dom.element(root, &W::body()) else {
        return;
    };
    let kids: Vec<NodeId> = dom
    ...
    let is_pure_i_content = |p: NodeId| {
        if dom.name(p) != Some(W::p()) || para_has_no_text(dom, p) {
            return false;
        }
        let body_kids: Vec<NodeId> = dom
            .elements(p, None)
    ...
    // Body starts pure-I (wholesale B list), not lead MIX/pure-D.
    let mut first_content = None;
    for &p in &kids {
        if dom.name(p) != Some(W::p()) {
            continue;
        }
        if para_is_empty_pure_ins(dom, p) {
            continue;
        }
        if para_has_no_text(dom, p) {
            continue;
        }
        first_content = Some(p);
        break;
    }
    let Some(fc) = first_content else {
        return;
    };
    if !is_pure_i_content(fc) {
        return;
    }

    // pure-I+numPr non-empty immediately followed by pure-D with short del.
    let mut insert_after: Option<NodeId> = None;
    for i in 0..kids.len().saturating_sub(1) {
        let cur = kids[i];
        let nxt = kids[i + 1];
        if !is_pure_i_content(cur) || !has_numpr(cur) {
            continue;
        }
        if dom.name(nxt) != Some(W::p()) || !para_is_pure_deleted(dom, nxt) {
            continue;
        }
        let del_text = para_revision_body_text(dom, nxt);
        if del_text.trim().is_empty() {
            continue;
        }
        let n = tok_n(&del_text);
        if !(1..=3).contains(&n) {
            continue;
        }
    ...
    let Some(cur) = insert_after else {
        return;
    };

    // Empty pure-I: pPr/spacing + rPr/ins (Word / tip inter-list spacer shape).
    let empty = dom.new_element(W::p());
    let ppr = dom.new_element(W::p_pr());
    let spacing = dom.new_element(W::name("spacing"));
    dom.set_attribute_value(spacing, &W::name("before"), Some("0"));
    dom.set_attribute_value(spacing, &W::name("after"), Some("0"));
    dom.set_attribute_value(spacing, &W::name("line"), Some("276"));
    dom.set_attribute_value(spacing, &W::name("lineRule"), Some("auto"));
    dom.add(ppr, spacing);
    let rpr = dom.new_element(W::r_pr());
    let ins = dom.new_element(W::ins());
    dom.set_attribute_value(ins, &W::author(), Some("Redline"));
    dom.set_attribute_value(ins, &W::id(), Some("0"));
    dom.set_attribute_value(ins, &W::date(), Some("1970-01-01T00:00:00Z"));
    dom.add(rpr, ins);
    dom.add(ppr, rpr);
    dom.add(empty, ppr);
    dom.add_after_self(cur, empty);
}

/// M265 (annot2): Word inserts a pure-D empty between pure-I "Enter duration"
/// (del pilcrow) and the following table. Tip goes duration→table. After
/// [`del_pilcrow_on_pure_ins_before_table`], insert an empty pure-D para
/// between **SDT** pure-I content and the next table when missing. Does not
/// insert before ordinary pure-I table titles (pirates).
pub fn insert_pure_d_empty_before_table_after_pure_ins(dom: &mut Dom, root: NodeId) {
    let Some(body) = dom.element(root, &W::body()) else {
        return;
    };
    let kids: Vec<NodeId> = dom
        ...
        .filter(|&k| dom.name(k) != Some(W::name("sectPr")))
        .collect();
    // Collect insert positions (after pure-I, before table) without mutating mid-iter.
    ...
    for i in 0..kids.len().saturating_sub(1) {
        ...
        // Pure-I body (ins present, no body del).
        let has_body_del = dom
            .elements(cur, None)
            .into_iter()
            .any(|c| dom.name(c) == Some(W::del()));
        ...
        let has_ins = !dom.descendants(cur, Some(&W::ins())).is_empty()
            || para_mark_revision(dom, cur, &W::ins());
        // After M264, pilcrow is del — still pure-I body.
        let has_body_ins = dom
            .elements(cur, None)
            .into_iter()
            .any(|c| dom.name(c) == Some(W::ins()))
            || !dom.descendants(cur, Some(&W::ins())).is_empty();
        if !has_body_ins && !has_ins {
            // Content pure-I may only have ins inside sdt.
            if dom.descendants(cur, Some(&W::name("sdt"))).is_empty() {
                continue;
            }
        }
        // Del pilcrow expected (M264) — only SDT pure-I content gets it.
        if !para_mark_revision(dom, cur, &W::del()) {
            continue;
        }
        if dom.descendants(cur, Some(&W::name("sdt"))).is_empty() {
            continue;
        }
        after_nodes.push(cur);
    }
    for cur in after_nodes {
        // Skip if already have empty pure-D between cur and next tbl.
        // (next sibling check after prior inserts)
        let siblings = dom.elements(body, None);
        let Some(pos) = siblings.iter().position(|&s| s == cur) else {
            continue;
        };
        ...
        if dom.name(nxt) != Some(W::name("tbl")) {
            continue; // something already between
        }
        // Build pure-D empty: p with pPr/rPr/del.
        let empty = dom.new_element(W::p());
        let ppr = dom.new_element(W::p_pr());
        let rpr = dom.new_element(W::r_pr());
        let del = dom.new_element(W::del());
...
/// M266 (pirates×table_left_indent): pure-D multi-row table under B props
/// (`tblPrChange` holds A's old props) splits at the following pure-I empty
/// table's row count. Word: 2+3 pure-D (Flats/Square | Circle/Cone/triangle)
/// then pure-I empty B table. First slice keeps live B props+tblPrChange;
/// remainder gets A's props from the change record.
pub fn split_pure_del_table_at_following_pure_ins_rows(dom: &mut Dom, root: NodeId) {
    let Some(body) = dom.element(root, &W::body()) else {
        return;
    };
    ...
    let mut plans: Vec<(NodeId, usize, NodeId)> = Vec::new(); // (del_tbl, split_at, old_tbl_pr)
    for (i, &t) in kids.iter().enumerate() {
        if dom.name(t) != Some(tbl_name.clone()) {
            continue;
        }
        if !table_is_fully_content_deleted(dom, t) {
            continue;
        }
        let rows = dom.elements(t, Some(&tr_name));
        let n = rows.len();
        if n < 3 {
            continue;
        }
        let Some(tblpr) = dom.element(t, &W::tbl_pr()) else {
            continue;
        };
        let Some(pr_change) = dom.element(tblpr, &W::name("tblPrChange")) else {
            continue;
        };
        let Some(old_pr) = dom.element(pr_change, &W::tbl_pr()) else {
            continue;
        };
        // Following pure-I empty table with m rows, 1≤m<n (skip pure-I paras).
        let mut m_found: Option<usize> = None;
        for &u in &kids[i + 1..] {
            if dom.name(u) == Some(W::p()) {
                continue;
            }
            if dom.name(u) != Some(tbl_name.clone()) {
                break;
            }
            if table_is_pure_ins_empty(dom, u) {
                let m = dom.elements(u, Some(&tr_name)).len();
                if (1..n).contains(&m) {
                    m_found = Some(m);
                }
            }
        ...
        let Some(m) = m_found else {
            continue;
        };
        plans.push((t, m, old_pr));
    }

    for (t, m, old_pr) in plans {
        let rows = dom.elements(t, Some(&tr_name));
        if rows.len() <= m {
            continue;
        }
        let new_tbl = dom.new_element(tbl_name.clone());
        let new_pr = dom.clone_subtree(old_pr);
        for ch in dom.elements(new_pr, Some(&W::name("tblPrChange"))) {
            dom.remove(ch);
        }
        dom.add(new_tbl, new_pr);
        if let Some(grid) = dom.element(t, &W::name("tblGrid")) {
            let new_grid = if let Some(gc) = dom.element(grid, &W::name("tblGridChange")) {
                if let Some(old_g) = dom.element(gc, &W::name("tblGrid")) {
                    let g = dom.clone_subtree(old_g);
                    for ch in dom.elements(g, Some(&W::name("tblGridChange"))) {
                        dom.remove(ch);
                    }
                    g
                } else {
                    let g = dom.clone_subtree(grid);
                    for ch in dom.elements(g, Some(&W::name("tblGridChange"))) {
                        dom.remove(ch);
                    }
                    g
        ...
        dom.add_after_self(t, new_tbl);
    }
}

fn table_is_fully_content_deleted(dom: &Dom, tbl: NodeId) -> bool {
    let tr_name = W::name("tr");
    let rows = dom.elements(tbl, Some(&tr_name));
    if rows.is_empty() {
        return false;
    }
    // No real revision ins wrappers (avoid matching insideH etc. — use element walk).
    if !dom.descendants(tbl, Some(&W::ins())).is_empty() {
        return false;
    }
    if dom.descendants(tbl, Some(&W::del())).is_empty() {
        return false;
    }
    rows.iter().all(|&tr| {
        if let Some(trpr) = dom.element(tr, &W::name("trPr"))
            && dom.element(trpr, &W::del()).is_some()
        {
            return true;
        }
        let texts = dom.descendants(tr, Some(&W::t()));
        let del_texts = dom.descendants(tr, Some(&W::del_text()));
        texts.is_empty() && (!del_texts.is_empty() || table_row_text_empty(dom, tr))
    })
}

fn table_is_pure_ins_empty(dom: &Dom, tbl: NodeId) -> bool {
    if !dom.descendants(tbl, Some(&W::del())).is_empty() {
        return false;
    }
    if dom.descendants(tbl, Some(&W::ins())).is_empty() {
    ...
    let tr_name = W::name("tr");
    let rows = dom.elements(tbl, Some(&tr_name));
    !rows.is_empty() && rows.iter().all(|&tr| table_row_text_empty(dom, tr))
}

fn table_row_text_empty(dom: &Dom, tr: NodeId) -> bool {
    for t in dom.descendants(tr, Some(&W::t())) {
        if !dom.value(t).trim().is_empty() {
            return false;
        }
    }
    for t in dom.descendants(tr, Some(&W::del_text())) {
        if !dom.value(t).trim().is_empty() {
            return false;
        }
    }
