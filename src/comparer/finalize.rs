...
/// M388 (memo×nda ~46): after free-mesh title MIX, Word parks short pure-D
/// memo headers (TO:/FROM:/DATE:/RE:) before pure-I body. Eng dumps pure-I
/// NDA first and leaves headers midstream. Splice a short pure-D header
/// cluster to immediately after the first MIX when it appears later.
pub fn splice_memo_headers_after_title_mix(dom: &mut Dom, root: NodeId) {
    let Some(body) = dom.element(root, &W::body()) else {
        return;
    };
    let kids: Vec<NodeId> = dom
    ...
    // First MIX = title free-mesh carrier.
    let mut mix_i: Option<usize> = None;
    for (i, &k) in kids.iter().enumerate() {
        if dom.name(k) != Some(W::p()) {
            continue;
        }
        let has_ins = !dom.descendants(k, Some(&W::ins())).is_empty()
            || para_mark_revision(dom, k, &W::ins());
        let has_del = !dom.descendants(k, Some(&W::del())).is_empty()
            || para_mark_revision(dom, k, &W::del());
        if has_ins && has_del {
            mix_i = Some(i);
            break;
        }
    }
    let Some(mi) = mix_i else {
        return;
    };
    // First pure-I body after MIX.
    let mut first_ins: Option<usize> = None;
    for (i, &k) in kids.iter().enumerate().skip(mi + 1) {
        if dom.name(k) == Some(W::p()) && para_is_pure_inserted(dom, k) && !para_has_no_text(dom, k)
        {
            first_ins = Some(i);
            break;
        }
    }
    let Some(fi) = first_ins else {
        return;
    };
    // Pure-D / mixed-del "TO:" header later than first pure-I.
    let mut to_i: Option<usize> = None;
    for (i, &k) in kids.iter().enumerate().skip(fi + 1) {
        if dom.name(k) != Some(W::p()) {
            continue;
        }
        let t = para_revision_body_text(dom, k).to_ascii_lowercase();
        let has_del = !dom.descendants(k, Some(&W::del())).is_empty()
            || para_mark_revision(dom, k, &W::del());
        if has_del && t.contains("to:") && t.contains("general counsel") {
            to_i = Some(i);
            break;
        }
    }
    let Some(ti) = to_i else {
        return;
    };
    // Collect short pure-D / del-bearing header cluster starting at ti
    // (TO/FROM/DATE/RE + short empties), stop at long pure-D prose or pure-I.
    let mut end = ti;
    while end < kids.len() {
        let k = kids[end];
        if dom.name(k) != Some(W::p()) {
        ...
        // Allow MIX (ins+del) only for short header residues.
        let alnum = para_body_alnum_len(dom, k);
        let t = para_revision_body_text(dom, k).to_ascii_lowercase();
        let headerish = alnum == 0
            || alnum <= 40
            || t.contains("to:")
            || t.contains("from:")
            || t.contains("date:")
        ...
        // Long pure-I body mixed in stops the cluster.
        if has_ins && alnum > 40 && !t.contains("to:") && !t.contains("from:") {
            break;
        }
        end += 1;
        if end - ti > 12 {
    ...
    let to_move: Vec<NodeId> = kids[ti..end].to_vec();
    let anchor = kids[mi];
    for &n in &to_move {
        if dom.parent(n).is_some() {
            dom.remove(n);
