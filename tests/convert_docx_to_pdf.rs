...
#[test]
fn pie_chart_series_are_filled_wedges() {
    let body = "<w:p><w:r><w:drawing><wp:inline>\
         <wp:extent cx=\"5486400\" cy=\"3200400\"/>\
         <a:graphic><a:graphicData \
           uri=\"http://schemas.openxmlformats.org/drawingml/2006/chart\">\
    ...
    let pdf = docx_to_pdf(&chart_docx(body, chart)).expect("convert pie chart");
    let hay = String::from_utf8_lossy(&pdf);
    assert!(
        pdf_has_filled_polygon(&hay),
        "pie slices must be FillPoly (h f), not only bar re; tail {}",
        &hay[hay.len().saturating_sub(400)..]
    );
}
...
fn line_chart_series_are_stroked_polylines() {
    ...
    let pdf = docx_to_pdf(&chart_docx(body, chart)).expect("convert line chart");
    let hay = String::from_utf8_lossy(&pdf);
    let re_count = hay.matches(" re f").count();
    assert!(
        hay.contains(" l S"),
        "line series must stroke polylines; tail {}",
        &hay[hay.len().saturating_sub(400)..]
    );
    assert_eq!(
        re_count,
        1,
        "line must not paint bar FillRects; re f count={re_count} tail {}",
        &hay[hay.len().saturating_sub(400)..]
    );
}

#[test]
fn area_chart_series_are_filled_polygons() {
    let body = "<w:p><w:r><w:drawing><wp:inline>\
         <wp:extent cx=\"5486400\" cy=\"3200400\"/>\
         <a:graphic><a:graphicData \
           uri=\"http://schemas.openxmlformats.org/drawingml/2006/chart\">\
    ...
    let pdf = docx_to_pdf(&chart_docx(body, chart)).expect("convert area chart");
    let hay = String::from_utf8_lossy(&pdf);
    let re_count = hay.matches(" re f").count();
    assert!(
        pdf_has_filled_polygon(&hay),
        "area series must fill a polygon (h f), not bar re; tail {}",
        &hay[hay.len().saturating_sub(400)..]
    );
    assert_eq!(
        re_count,
        1,
        "area must not paint bar FillRects; re f count={re_count} tail {}",
        &hay[hay.len().saturating_sub(400)..]
    );
}

#[test]
fn scatter_chart_points_are_filled_markers() {
    let body = "<w:p><w:r><w:drawing><wp:inline>\
         <wp:extent cx=\"5486400\" cy=\"3200400\"/>\
         <a:graphic><a:graphicData \
           uri=\"http://schemas.openxmlformats.org/drawingml/2006/chart\">\
    ...
    let pdf = docx_to_pdf(&chart_docx(body, chart)).expect("convert scatter chart");
    let hay = String::from_utf8_lossy(&pdf);
    let re_count = hay.matches(" re f").count();
    assert!(
        pdf_has_filled_polygon(&hay),
        "scatter markers must be FillPoly (h f), not bar re; tail {}",
        &hay[hay.len().saturating_sub(400)..]
    );
    assert_eq!(
        re_count,
...
#[test]
fn star7_prst_fills_a_polygon_not_a_rect() {
    let body = preset_shape_body("star7");
    let pdf = docx_to_pdf(&drawing_docx(&body)).expect("convert star7");
    let hay = String::from_utf8_lossy(&pdf);
    assert!(
        pdf_has_filled_polygon(&hay),
        "star7 must fill a polygon (h f), not only re rects; tail {}",
        &hay[hay.len().saturating_sub(400)..]
    );
    let rects = pdf_fill_boxes_in(&hay, 1.0, 0.0, 0.0);
    assert!(
...
#[test]
fn star8_prst_fills_a_polygon_not_a_rect() {
    let body = preset_shape_body("star8");
    let pdf = docx_to_pdf(&drawing_docx(&body)).expect("convert star8");
    let hay = String::from_utf8_lossy(&pdf);
    assert!(
        pdf_has_filled_polygon(&hay),
        "star8 must fill a polygon (h f), not only re rects; tail {}",
        &hay[hay.len().saturating_sub(400)..]
    );
    let rects = pdf_fill_boxes_in(&hay, 1.0, 0.0, 0.0);
    assert!(
...
#[test]
fn star10_prst_fills_a_polygon_not_a_rect() {
    let body = preset_shape_body("star10");
    let pdf = docx_to_pdf(&drawing_docx(&body)).expect("convert star10");
    let hay = String::from_utf8_lossy(&pdf);
    assert!(
        pdf_has_filled_polygon(&hay),
        "star10 must fill a polygon (h f), not only re rects; tail {}",
        &hay[hay.len().saturating_sub(400)..]
    );
    let rects = pdf_fill_boxes_in(&hay, 1.0, 0.0, 0.0);
    assert!(
...
#[test]
fn star12_prst_fills_a_polygon_not_a_rect() {
    let body = preset_shape_body("star12");
    let pdf = docx_to_pdf(&drawing_docx(&body)).expect("convert star12");
    let hay = String::from_utf8_lossy(&pdf);
    assert!(
        pdf_has_filled_polygon(&hay),
        "star12 must fill a polygon (h f), not only re rects; tail {}",
        &hay[hay.len().saturating_sub(400)..]
    );
    let rects = pdf_fill_boxes_in(&hay, 1.0, 0.0, 0.0);
    assert!(
...
#[test]
fn star24_prst_fills_a_polygon_not_a_rect() {
    let body = preset_shape_body("star24");
    let pdf = docx_to_pdf(&drawing_docx(&body)).expect("convert star24");
    let hay = String::from_utf8_lossy(&pdf);
    assert!(
        pdf_has_filled_polygon(&hay),
        "star24 must fill a polygon (h f), not only re rects; tail {}",
        &hay[hay.len().saturating_sub(400)..]
    );
    let rects = pdf_fill_boxes_in(&hay, 1.0, 0.0, 0.0);
    assert!(
...
#[test]
fn star32_prst_fills_a_polygon_not_a_rect() {
    let body = preset_shape_body("star32");
    let pdf = docx_to_pdf(&drawing_docx(&body)).expect("convert star32");
    let hay = String::from_utf8_lossy(&pdf);
    assert!(
        pdf_has_filled_polygon(&hay),
        "star32 must fill a polygon (h f), not only re rects; tail {}",
        &hay[hay.len().saturating_sub(400)..]
    );
    let rects = pdf_fill_boxes_in(&hay, 1.0, 0.0, 0.0);
    assert!(
...
#[test]
fn flow_chart_document_prst_fills_a_polygon_not_a_rect() {
    let body = preset_shape_body("flowChartDocument");
    let pdf = docx_to_pdf(&drawing_docx(&body)).expect("convert flowChartDocument");
    let hay = String::from_utf8_lossy(&pdf);
    assert!(
        pdf_has_filled_polygon(&hay),
        "flowChartDocument must fill a polygon (h f), not only re rects; tail {}",
        &hay[hay.len().saturating_sub(400)..]
    );
    let rects = pdf_fill_boxes_in(&hay, 1.0, 0.0, 0.0);
    assert!(
...
#[test]
fn flow_chart_offpage_connector_prst_fills_a_polygon_not_a_rect() {
    let body = preset_shape_body("flowChartOffpageConnector");
    let pdf = docx_to_pdf(&drawing_docx(&body)).expect("convert flowChartOffpageConnector");
    let hay = String::from_utf8_lossy(&pdf);
    assert!(
        pdf_has_filled_polygon(&hay),
        "flowChartOffpageConnector must fill a polygon (h f), not only re rects; tail {}",
        &hay[hay.len().saturating_sub(400)..]
    );
    let rects = pdf_fill_boxes_in(&hay, 1.0, 0.0, 0.0);
    assert!(
...
#[test]
fn flow_chart_delay_prst_fills_a_polygon_not_a_rect() {
    let body = preset_shape_body("flowChartDelay");
    let pdf = docx_to_pdf(&drawing_docx(&body)).expect("convert flowChartDelay");
    let hay = String::from_utf8_lossy(&pdf);
    assert!(
        pdf_has_filled_polygon(&hay),
        "flowChartDelay must fill a polygon (h f), not only re rects; tail {}",
        &hay[hay.len().saturating_sub(400)..]
    );
    let rects = pdf_fill_boxes_in(&hay, 1.0, 0.0, 0.0);
    assert!(
...
#[test]
fn flow_chart_manual_input_prst_fills_a_polygon_not_a_rect() {
    let body = preset_shape_body("flowChartManualInput");
    let pdf = docx_to_pdf(&drawing_docx(&body)).expect("convert flowChartManualInput");
    let hay = String::from_utf8_lossy(&pdf);
    assert!(
        pdf_has_filled_polygon(&hay),
        "flowChartManualInput must fill a polygon (h f), not only re rects; tail {}",
        &hay[hay.len().saturating_sub(400)..]
    );
    let rects = pdf_fill_boxes_in(&hay, 1.0, 0.0, 0.0);
    assert!(
...
#[test]
fn flow_chart_punched_card_prst_fills_a_polygon_not_a_rect() {
    let body = preset_shape_body("flowChartPunchedCard");
    let pdf = docx_to_pdf(&drawing_docx(&body)).expect("convert flowChartPunchedCard");
    let hay = String::from_utf8_lossy(&pdf);
    assert!(
        pdf_has_filled_polygon(&hay),
        "flowChartPunchedCard must fill a polygon (h f), not only re rects; tail {}",
        &hay[hay.len().saturating_sub(400)..]
    );
    let rects = pdf_fill_boxes_in(&hay, 1.0, 0.0, 0.0);
    assert!(
...
#[test]
fn flow_chart_preparation_prst_fills_a_polygon_not_a_rect() {
    let body = preset_shape_body("flowChartPreparation");
    let pdf = docx_to_pdf(&drawing_docx(&body)).expect("convert flowChartPreparation");
    let hay = String::from_utf8_lossy(&pdf);
    assert!(
        pdf_has_filled_polygon(&hay),
        "flowChartPreparation must fill a polygon (h f), not only re rects; tail {}",
        &hay[hay.len().saturating_sub(400)..]
    );
    let rects = pdf_fill_boxes_in(&hay, 1.0, 0.0, 0.0);
    assert!(
...
#[test]
fn flow_chart_extract_prst_fills_a_polygon_not_a_rect() {
    let body = preset_shape_body("flowChartExtract");
    let pdf = docx_to_pdf(&drawing_docx(&body)).expect("convert flowChartExtract");
    let hay = String::from_utf8_lossy(&pdf);
    assert!(
        pdf_has_filled_polygon(&hay),
        "flowChartExtract must fill a polygon (h f), not only re rects; tail {}",
        &hay[hay.len().saturating_sub(400)..]
    );
    let rects = pdf_fill_boxes_in(&hay, 1.0, 0.0, 0.0);
    assert!(
...
#[test]
fn flow_chart_merge_prst_fills_a_polygon_not_a_rect() {
    let body = preset_shape_body("flowChartMerge");
    let pdf = docx_to_pdf(&drawing_docx(&body)).expect("convert flowChartMerge");
    let hay = String::from_utf8_lossy(&pdf);
    assert!(
        pdf_has_filled_polygon(&hay),
        "flowChartMerge must fill a polygon (h f), not only re rects; tail {}",
        &hay[hay.len().saturating_sub(400)..]
    );
    let rects = pdf_fill_boxes_in(&hay, 1.0, 0.0, 0.0);
    assert!(
...
#[test]
fn flow_chart_collate_prst_fills_a_polygon_not_a_rect() {
    let body = preset_shape_body("flowChartCollate");
    let pdf = docx_to_pdf(&drawing_docx(&body)).expect("convert flowChartCollate");
    let hay = String::from_utf8_lossy(&pdf);
    assert!(
        pdf_has_filled_polygon(&hay),
        "flowChartCollate must fill a polygon (h f), not only re rects; tail {}",
        &hay[hay.len().saturating_sub(400)..]
    );
    let rects = pdf_fill_boxes_in(&hay, 1.0, 0.0, 0.0);
    assert!(
...
#[test]
fn double_wave_prst_fills_a_polygon_not_a_rect() {
    let body = preset_shape_body("doubleWave");
    let pdf = docx_to_pdf(&drawing_docx(&body)).expect("convert doubleWave");
    let hay = String::from_utf8_lossy(&pdf);
    assert!(
        pdf_has_filled_polygon(&hay),
        "doubleWave must fill a polygon (h f), not only re rects; tail {}",
        &hay[hay.len().saturating_sub(400)..]
    );
    let rects = pdf_fill_boxes_in(&hay, 1.0, 0.0, 0.0);
    assert!(
...
#[test]
fn flow_chart_display_prst_fills_a_polygon_not_a_rect() {
    let body = preset_shape_body("flowChartDisplay");
    let pdf = docx_to_pdf(&drawing_docx(&body)).expect("convert flowChartDisplay");
    let hay = String::from_utf8_lossy(&pdf);
    assert!(
        pdf_has_filled_polygon(&hay),
        "flowChartDisplay must fill a polygon (h f), not only re rects; tail {}",
        &hay[hay.len().saturating_sub(400)..]
    );
    let rects = pdf_fill_boxes_in(&hay, 1.0, 0.0, 0.0);
    assert!(
...
#[test]
fn flow_chart_input_output_prst_fills_a_polygon_not_a_rect() {
    let body = preset_shape_body("flowChartInputOutput");
    let pdf = docx_to_pdf(&drawing_docx(&body)).expect("convert flowChartInputOutput");
    let hay = String::from_utf8_lossy(&pdf);
    assert!(
        pdf_has_filled_polygon(&hay),
        "flowChartInputOutput must fill a polygon (h f), not only re rects; tail {}",
        &hay[hay.len().saturating_sub(400)..]
    );
    let rects = pdf_fill_boxes_in(&hay, 1.0, 0.0, 0.0);
    assert!(
...
#[test]
fn flow_chart_manual_operation_prst_fills_a_polygon_not_a_rect() {
    let body = preset_shape_body("flowChartManualOperation");
    let pdf = docx_to_pdf(&drawing_docx(&body)).expect("convert flowChartManualOperation");
    let hay = String::from_utf8_lossy(&pdf);
    assert!(
        pdf_has_filled_polygon(&hay),
        "flowChartManualOperation must fill a polygon (h f), not only re rects; tail {}",
        &hay[hay.len().saturating_sub(400)..]
    );
    let rects = pdf_fill_boxes_in(&hay, 1.0, 0.0, 0.0);
    assert!(
...
#[test]
fn flow_chart_sort_prst_fills_a_polygon_not_a_rect() {
    let body = preset_shape_body("flowChartSort");
    let pdf = docx_to_pdf(&drawing_docx(&body)).expect("convert flowChartSort");
    let hay = String::from_utf8_lossy(&pdf);
    assert!(
        pdf_has_filled_polygon(&hay),
        "flowChartSort must fill a polygon (h f), not only re rects; tail {}",
        &hay[hay.len().saturating_sub(400)..]
    );
    let rects = pdf_fill_boxes_in(&hay, 1.0, 0.0, 0.0);
    assert!(
...
#[test]
fn flow_chart_offline_storage_prst_fills_a_polygon_not_a_rect() {
    let body = preset_shape_body("flowChartOfflineStorage");
    let pdf = docx_to_pdf(&drawing_docx(&body)).expect("convert flowChartOfflineStorage");
    let hay = String::from_utf8_lossy(&pdf);
    assert!(
        pdf_has_filled_polygon(&hay),
        "flowChartOfflineStorage must fill a polygon (h f), not only re rects; tail {}",
        &hay[hay.len().saturating_sub(400)..]
    );
    let rects = pdf_fill_boxes_in(&hay, 1.0, 0.0, 0.0);
    assert!(
...
#[test]
fn flow_chart_online_storage_prst_fills_a_polygon_not_a_rect() {
    let body = preset_shape_body("flowChartOnlineStorage");
    let pdf = docx_to_pdf(&drawing_docx(&body)).expect("convert flowChartOnlineStorage");
    let hay = String::from_utf8_lossy(&pdf);
    assert!(
        pdf_has_filled_polygon(&hay),
        "flowChartOnlineStorage must fill a polygon (h f), not only re rects; tail {}",
        &hay[hay.len().saturating_sub(400)..]
    );
    let rects = pdf_fill_boxes_in(&hay, 1.0, 0.0, 0.0);
    assert!(
...
#[test]
fn flow_chart_connector_prst_fills_a_polygon_not_a_rect() {
    let body = preset_shape_body("flowChartConnector");
    let pdf = docx_to_pdf(&drawing_docx(&body)).expect("convert flowChartConnector");
    let hay = String::from_utf8_lossy(&pdf);
    assert!(
        pdf_has_filled_polygon(&hay),
        "flowChartConnector must fill a polygon (h f), not only re rects; tail {}",
        &hay[hay.len().saturating_sub(400)..]
    );
    let rects = pdf_fill_boxes_in(&hay, 1.0, 0.0, 0.0);
    assert!(
...
#[test]
fn flow_chart_punched_tape_prst_fills_a_polygon_not_a_rect() {
    let body = preset_shape_body("flowChartPunchedTape");
    let pdf = docx_to_pdf(&drawing_docx(&body)).expect("convert flowChartPunchedTape");
    let hay = String::from_utf8_lossy(&pdf);
    assert!(
        pdf_has_filled_polygon(&hay),
        "flowChartPunchedTape must fill a polygon (h f), not only re rects; tail {}",
        &hay[hay.len().saturating_sub(400)..]
    );
    let rects = pdf_fill_boxes_in(&hay, 1.0, 0.0, 0.0);
    assert!(
...
#[test]
fn flow_chart_alternate_process_prst_fills_a_polygon_not_a_rect() {
    let body = preset_shape_body("flowChartAlternateProcess");
    let pdf = docx_to_pdf(&drawing_docx(&body)).expect("convert flowChartAlternateProcess");
    let hay = String::from_utf8_lossy(&pdf);
    assert!(
        pdf_has_filled_polygon(&hay),
        "flowChartAlternateProcess must fill a polygon (h f), not only re rects; tail {}",
        &hay[hay.len().saturating_sub(400)..]
    );
    let rects = pdf_fill_boxes_in(&hay, 1.0, 0.0, 0.0);
    assert!(
...
#[test]
fn flow_chart_or_prst_fills_a_polygon_not_a_rect() {
    let body = preset_shape_body("flowChartOr");
    let pdf = docx_to_pdf(&drawing_docx(&body)).expect("convert flowChartOr");
    let hay = String::from_utf8_lossy(&pdf);
    assert!(
        pdf_has_filled_polygon(&hay),
        "flowChartOr must fill a polygon (h f), not only re rects; tail {}",
        &hay[hay.len().saturating_sub(400)..]
    );
    let rects = pdf_fill_boxes_in(&hay, 1.0, 0.0, 0.0);
    assert!(
...
#[test]
fn flow_chart_summing_junction_prst_fills_a_polygon_not_a_rect() {
    let body = preset_shape_body("flowChartSummingJunction");
    let pdf = docx_to_pdf(&drawing_docx(&body)).expect("convert flowChartSummingJunction");
    let hay = String::from_utf8_lossy(&pdf);
    assert!(
        pdf_has_filled_polygon(&hay),
        "flowChartSummingJunction must fill a polygon (h f), not only re rects; tail {}",
        &hay[hay.len().saturating_sub(400)..]
    );
    let rects = pdf_fill_boxes_in(&hay, 1.0, 0.0, 0.0);
    assert!(
...
#[test]
fn flow_chart_internal_storage_prst_fills_a_polygon_not_a_rect() {
    let body = preset_shape_body("flowChartInternalStorage");
    let pdf = docx_to_pdf(&drawing_docx(&body)).expect("convert flowChartInternalStorage");
    let hay = String::from_utf8_lossy(&pdf);
    assert!(
        pdf_has_filled_polygon(&hay),
        "flowChartInternalStorage must fill a polygon (h f), not only re rects; tail {}",
        &hay[hay.len().saturating_sub(400)..]
    );
    let rects = pdf_fill_boxes_in(&hay, 1.0, 0.0, 0.0);
    assert!(
...
#[test]
fn flow_chart_predefined_process_prst_fills_a_polygon_not_a_rect() {
    let body = preset_shape_body("flowChartPredefinedProcess");
    let pdf = docx_to_pdf(&drawing_docx(&body)).expect("convert flowChartPredefinedProcess");
    let hay = String::from_utf8_lossy(&pdf);
    assert!(
        pdf_has_filled_polygon(&hay),
        "flowChartPredefinedProcess must fill a polygon (h f), not only re rects; tail {}",
        &hay[hay.len().saturating_sub(400)..]
    );
    let rects = pdf_fill_boxes_in(&hay, 1.0, 0.0, 0.0);
    assert!(
...
#[test]
fn flow_chart_magnetic_disk_prst_fills_a_polygon_not_a_rect() {
    let body = preset_shape_body("flowChartMagneticDisk");
    let pdf = docx_to_pdf(&drawing_docx(&body)).expect("convert flowChartMagneticDisk");
    let hay = String::from_utf8_lossy(&pdf);
    assert!(
        pdf_has_filled_polygon(&hay),
        "flowChartMagneticDisk must fill a polygon (h f), not only re rects; tail {}",
        &hay[hay.len().saturating_sub(400)..]
    );
    let rects = pdf_fill_boxes_in(&hay, 1.0, 0.0, 0.0);
    assert!(
...
#[test]
fn flow_chart_magnetic_drum_prst_fills_a_polygon_not_a_rect() {
    let body = preset_shape_body("flowChartMagneticDrum");
    let pdf = docx_to_pdf(&drawing_docx(&body)).expect("convert flowChartMagneticDrum");
    let hay = String::from_utf8_lossy(&pdf);
    assert!(
        pdf_has_filled_polygon(&hay),
        "flowChartMagneticDrum must fill a polygon (h f), not only re rects; tail {}",
        &hay[hay.len().saturating_sub(400)..]
    );
    let rects = pdf_fill_boxes_in(&hay, 1.0, 0.0, 0.0);
    assert!(
...
#[test]
fn flow_chart_magnetic_tape_prst_fills_a_polygon_not_a_rect() {
    let body = preset_shape_body("flowChartMagneticTape");
    let pdf = docx_to_pdf(&drawing_docx(&body)).expect("convert flowChartMagneticTape");
    let hay = String::from_utf8_lossy(&pdf);
    assert!(
        pdf_has_filled_polygon(&hay),
        "flowChartMagneticTape must fill a polygon (h f), not only re rects; tail {}",
        &hay[hay.len().saturating_sub(400)..]
    );
    let rects = pdf_fill_boxes_in(&hay, 1.0, 0.0, 0.0);
    assert!(
...
#[test]
fn flow_chart_multidocument_prst_fills_a_polygon_not_a_rect() {
    let body = preset_shape_body("flowChartMultidocument");
    let pdf = docx_to_pdf(&drawing_docx(&body)).expect("convert flowChartMultidocument");
    let hay = String::from_utf8_lossy(&pdf);
    assert!(
        pdf_has_filled_polygon(&hay),
        "flowChartMultidocument must fill a polygon (h f), not only re rects; tail {}",
        &hay[hay.len().saturating_sub(400)..]
    );
    let rects = pdf_fill_boxes_in(&hay, 1.0, 0.0, 0.0);
    assert!(
...
#[test]
fn moon_prst_fills_a_polygon_not_a_rect() {
    let body = preset_shape_body("moon");
    let pdf = docx_to_pdf(&drawing_docx(&body)).expect("convert moon");
    let hay = String::from_utf8_lossy(&pdf);
    assert!(
        pdf_has_filled_polygon(&hay),
        "moon must fill a polygon (h f), not only re rects; tail {}",
        &hay[hay.len().saturating_sub(400)..]
    );
    let rects = pdf_fill_boxes_in(&hay, 1.0, 0.0, 0.0);
    assert!(
...
#[test]
fn circular_arrow_prst_fills_a_polygon_not_a_rect() {
    let body = preset_shape_body("circularArrow");
    let pdf = docx_to_pdf(&drawing_docx(&body)).expect("convert circularArrow");
    let hay = String::from_utf8_lossy(&pdf);
    assert!(
        pdf_has_filled_polygon(&hay),
        "circularArrow must fill a polygon (h f), not only re rects; tail {}",
        &hay[hay.len().saturating_sub(400)..]
    );
    let rects = pdf_fill_boxes_in(&hay, 1.0, 0.0, 0.0);
    assert!(
...
#[test]
fn gear6_prst_fills_a_polygon_not_a_rect() {
    let body = preset_shape_body("gear6");
    let pdf = docx_to_pdf(&drawing_docx(&body)).expect("convert gear6");
    let hay = String::from_utf8_lossy(&pdf);
    assert!(
        pdf_has_filled_polygon(&hay),
        "gear6 must fill a polygon (h f), not only re rects; tail {}",
        &hay[hay.len().saturating_sub(400)..]
    );
    let rects = pdf_fill_boxes_in(&hay, 1.0, 0.0, 0.0);
    assert!(
...
#[test]
fn smiley_face_prst_fills_a_polygon_not_a_rect() {
    let body = preset_shape_body("smileyFace");
    let pdf = docx_to_pdf(&drawing_docx(&body)).expect("convert smileyFace");
    let hay = String::from_utf8_lossy(&pdf);
    assert!(
        pdf_has_filled_polygon(&hay),
        "smileyFace must fill a polygon (h f), not only re rects; tail {}",
        &hay[hay.len().saturating_sub(400)..]
    );
    let rects = pdf_fill_boxes_in(&hay, 1.0, 0.0, 0.0);
    assert!(
...
#[test]
fn gear9_prst_fills_a_polygon_not_a_rect() {
    let body = preset_shape_body("gear9");
    let pdf = docx_to_pdf(&drawing_docx(&body)).expect("convert gear9");
    let hay = String::from_utf8_lossy(&pdf);
    assert!(
        pdf_has_filled_polygon(&hay),
        "gear9 must fill a polygon (h f), not only re rects; tail {}",
        &hay[hay.len().saturating_sub(400)..]
    );
    let rects = pdf_fill_boxes_in(&hay, 1.0, 0.0, 0.0);
    assert!(
...
#[test]
fn teardrop_prst_fills_a_polygon_not_a_rect() {
    let body = preset_shape_body("teardrop");
    let pdf = docx_to_pdf(&drawing_docx(&body)).expect("convert teardrop");
    let hay = String::from_utf8_lossy(&pdf);
    assert!(
        pdf_has_filled_polygon(&hay),
        "teardrop must fill a polygon (h f), not only re rects; tail {}",
        &hay[hay.len().saturating_sub(400)..]
    );
    let rects = pdf_fill_boxes_in(&hay, 1.0, 0.0, 0.0);
    assert!(
...
#[test]
fn no_smoking_prst_fills_a_polygon_not_a_rect() {
    let body = preset_shape_body("noSmoking");
    let pdf = docx_to_pdf(&drawing_docx(&body)).expect("convert noSmoking");
    let hay = String::from_utf8_lossy(&pdf);
    assert!(
        pdf_has_filled_polygon(&hay),
        "noSmoking must fill a polygon (h f), not only re rects; tail {}",
        &hay[hay.len().saturating_sub(400)..]
    );
    let rects = pdf_fill_boxes_in(&hay, 1.0, 0.0, 0.0);
    assert!(
...
#[test]
fn plaque_prst_fills_a_polygon_not_a_rect() {
    let body = preset_shape_body("plaque");
    let pdf = docx_to_pdf(&drawing_docx(&body)).expect("convert plaque");
    let hay = String::from_utf8_lossy(&pdf);
    assert!(
        pdf_has_filled_polygon(&hay),
        "plaque must fill a polygon (h f), not only re rects; tail {}",
        &hay[hay.len().saturating_sub(400)..]
    );
    let rects = pdf_fill_boxes_in(&hay, 1.0, 0.0, 0.0);
    assert!(
...
#[test]
fn left_circular_arrow_prst_fills_a_polygon_not_a_rect() {
    let body = preset_shape_body("leftCircularArrow");
    let pdf = docx_to_pdf(&drawing_docx(&body)).expect("convert leftCircularArrow");
    let hay = String::from_utf8_lossy(&pdf);
    assert!(
        pdf_has_filled_polygon(&hay),
        "leftCircularArrow must fill a polygon (h f), not only re rects; tail {}",
        &hay[hay.len().saturating_sub(400)..]
    );
    let rects = pdf_fill_boxes_in(&hay, 1.0, 0.0, 0.0);
    assert!(
...
#[test]
fn block_arc_prst_fills_a_polygon_not_a_rect() {
    let body = preset_shape_body("blockArc");
    let pdf = docx_to_pdf(&drawing_docx(&body)).expect("convert blockArc");
    let hay = String::from_utf8_lossy(&pdf);
    assert!(
        pdf_has_filled_polygon(&hay),
        "blockArc must fill a polygon (h f), not only re rects; tail {}",
        &hay[hay.len().saturating_sub(400)..]
    );
    let rects = pdf_fill_boxes_in(&hay, 1.0, 0.0, 0.0);
    assert!(
...
#[test]
fn chord_prst_fills_a_polygon_not_a_rect() {
    let body = preset_shape_body("chord");
    let pdf = docx_to_pdf(&drawing_docx(&body)).expect("convert chord");
    let hay = String::from_utf8_lossy(&pdf);
    assert!(
        pdf_has_filled_polygon(&hay),
        "chord must fill a polygon (h f), not only re rects; tail {}",
        &hay[hay.len().saturating_sub(400)..]
    );
    let rects = pdf_fill_boxes_in(&hay, 1.0, 0.0, 0.0);
    assert!(
...
#[test]
fn bevel_prst_fills_a_polygon_not_a_rect() {
    let body = preset_shape_body("bevel");
    let pdf = docx_to_pdf(&drawing_docx(&body)).expect("convert bevel");
    let hay = String::from_utf8_lossy(&pdf);
    assert!(
        pdf_has_filled_polygon(&hay),
        "bevel must fill a polygon (h f), not only re rects; tail {}",
        &hay[hay.len().saturating_sub(400)..]
    );
    let rects = pdf_fill_boxes_in(&hay, 1.0, 0.0, 0.0);
    assert!(
...
#[test]
fn arc_prst_fills_a_polygon_not_a_rect() {
    let body = preset_shape_body("arc");
    let pdf = docx_to_pdf(&drawing_docx(&body)).expect("convert arc");
    let hay = String::from_utf8_lossy(&pdf);
    assert!(
        pdf_has_filled_polygon(&hay),
        "arc must fill a polygon (h f), not only re rects; tail {}",
        &hay[hay.len().saturating_sub(400)..]
    );
    let rects = pdf_fill_boxes_in(&hay, 1.0, 0.0, 0.0);
    assert!(
...
#[test]
fn left_bracket_prst_fills_a_polygon_not_a_rect() {
    let body = preset_shape_body("leftBracket");
    let pdf = docx_to_pdf(&drawing_docx(&body)).expect("convert leftBracket");
    let hay = String::from_utf8_lossy(&pdf);
    assert!(
        pdf_has_filled_polygon(&hay),
        "leftBracket must fill a polygon (h f), not only re rects; tail {}",
        &hay[hay.len().saturating_sub(400)..]
    );
    let rects = pdf_fill_boxes_in(&hay, 1.0, 0.0, 0.0);
    assert!(
...
#[test]
fn right_bracket_prst_fills_a_polygon_not_a_rect() {
    let body = preset_shape_body("rightBracket");
    let pdf = docx_to_pdf(&drawing_docx(&body)).expect("convert rightBracket");
    let hay = String::from_utf8_lossy(&pdf);
    assert!(
        pdf_has_filled_polygon(&hay),
        "rightBracket must fill a polygon (h f), not only re rects; tail {}",
        &hay[hay.len().saturating_sub(400)..]
    );
    let rects = pdf_fill_boxes_in(&hay, 1.0, 0.0, 0.0);
    assert!(
...
#[test]
fn left_brace_prst_fills_a_polygon_not_a_rect() {
    let body = preset_shape_body("leftBrace");
    let pdf = docx_to_pdf(&drawing_docx(&body)).expect("convert leftBrace");
    let hay = String::from_utf8_lossy(&pdf);
    assert!(
        pdf_has_filled_polygon(&hay),
        "leftBrace must fill a polygon (h f), not only re rects; tail {}",
        &hay[hay.len().saturating_sub(400)..]
    );
    let rects = pdf_fill_boxes_in(&hay, 1.0, 0.0, 0.0);
    assert!(
...
#[test]
fn right_brace_prst_fills_a_polygon_not_a_rect() {
    let body = preset_shape_body("rightBrace");
    let pdf = docx_to_pdf(&drawing_docx(&body)).expect("convert rightBrace");
    let hay = String::from_utf8_lossy(&pdf);
    assert!(
        pdf_has_filled_polygon(&hay),
        "rightBrace must fill a polygon (h f), not only re rects; tail {}",
        &hay[hay.len().saturating_sub(400)..]
    );
    let rects = pdf_fill_boxes_in(&hay, 1.0, 0.0, 0.0);
    assert!(
...
#[test]
fn brace_pair_prst_fills_a_polygon_not_a_rect() {
    let body = preset_shape_body("bracePair");
    let pdf = docx_to_pdf(&drawing_docx(&body)).expect("convert bracePair");
    let hay = String::from_utf8_lossy(&pdf);
    assert!(
        pdf_has_filled_polygon(&hay),
        "bracePair must fill a polygon (h f), not only re rects; tail {}",
        &hay[hay.len().saturating_sub(400)..]
    );
    let rects = pdf_fill_boxes_in(&hay, 1.0, 0.0, 0.0);
    assert!(
...
#[test]
fn bracket_pair_prst_fills_a_polygon_not_a_rect() {
    let body = preset_shape_body("bracketPair");
    let pdf = docx_to_pdf(&drawing_docx(&body)).expect("convert bracketPair");
    let hay = String::from_utf8_lossy(&pdf);
    assert!(
        pdf_has_filled_polygon(&hay),
        "bracketPair must fill a polygon (h f), not only re rects; tail {}",
        &hay[hay.len().saturating_sub(400)..]
    );
    let rects = pdf_fill_boxes_in(&hay, 1.0, 0.0, 0.0);
    assert!(
...
#[test]
fn snip1_rect_prst_fills_a_polygon_not_a_rect() {
    let body = preset_shape_body("snip1Rect");
    let pdf = docx_to_pdf(&drawing_docx(&body)).expect("convert snip1Rect");
    let hay = String::from_utf8_lossy(&pdf);
    assert!(
        pdf_has_filled_polygon(&hay),
        "snip1Rect must fill a polygon (h f), not only re rects; tail {}",
        &hay[hay.len().saturating_sub(400)..]
    );
    let rects = pdf_fill_boxes_in(&hay, 1.0, 0.0, 0.0);
    assert!(
...
#[test]
fn round1_rect_prst_fills_a_polygon_not_a_rect() {
    let body = preset_shape_body("round1Rect");
    let pdf = docx_to_pdf(&drawing_docx(&body)).expect("convert round1Rect");
    let hay = String::from_utf8_lossy(&pdf);
    assert!(
        pdf_has_filled_polygon(&hay),
        "round1Rect must fill a polygon (h f), not only re rects; tail {}",
        &hay[hay.len().saturating_sub(400)..]
    );
    let rects = pdf_fill_boxes_in(&hay, 1.0, 0.0, 0.0);
    assert!(
...
#[test]
fn snip2_same_rect_prst_fills_a_polygon_not_a_rect() {
    let body = preset_shape_body("snip2SameRect");
    let pdf = docx_to_pdf(&drawing_docx(&body)).expect("convert snip2SameRect");
    let hay = String::from_utf8_lossy(&pdf);
    assert!(
        pdf_has_filled_polygon(&hay),
        "snip2SameRect must fill a polygon (h f), not only re rects; tail {}",
        &hay[hay.len().saturating_sub(400)..]
    );
    let rects = pdf_fill_boxes_in(&hay, 1.0, 0.0, 0.0);
    assert!(
...
#[test]
fn round2_same_rect_prst_fills_a_polygon_not_a_rect() {
    let body = preset_shape_body("round2SameRect");
    let pdf = docx_to_pdf(&drawing_docx(&body)).expect("convert round2SameRect");
    let hay = String::from_utf8_lossy(&pdf);
    assert!(
        pdf_has_filled_polygon(&hay),
        "round2SameRect must fill a polygon (h f), not only re rects; tail {}",
        &hay[hay.len().saturating_sub(400)..]
    );
    let rects = pdf_fill_boxes_in(&hay, 1.0, 0.0, 0.0);
    assert!(
...
#[test]
fn snip2_diag_rect_prst_fills_a_polygon_not_a_rect() {
    let body = preset_shape_body("snip2DiagRect");
    let pdf = docx_to_pdf(&drawing_docx(&body)).expect("convert snip2DiagRect");
    let hay = String::from_utf8_lossy(&pdf);
    assert!(
        pdf_has_filled_polygon(&hay),
        "snip2DiagRect must fill a polygon (h f), not only re rects; tail {}",
        &hay[hay.len().saturating_sub(400)..]
    );
    let rects = pdf_fill_boxes_in(&hay, 1.0, 0.0, 0.0);
    assert!(
...
#[test]
fn round2_diag_rect_prst_fills_a_polygon_not_a_rect() {
    let body = preset_shape_body("round2DiagRect");
    let pdf = docx_to_pdf(&drawing_docx(&body)).expect("convert round2DiagRect");
    let hay = String::from_utf8_lossy(&pdf);
    assert!(
        pdf_has_filled_polygon(&hay),
        "round2DiagRect must fill a polygon (h f), not only re rects; tail {}",
        &hay[hay.len().saturating_sub(400)..]
    );
    let rects = pdf_fill_boxes_in(&hay, 1.0, 0.0, 0.0);
    assert!(
...
#[test]
fn ribbon_prst_fills_a_polygon_not_a_rect() {
    let body = preset_shape_body("ribbon");
    let pdf = docx_to_pdf(&drawing_docx(&body)).expect("convert ribbon");
    let hay = String::from_utf8_lossy(&pdf);
    assert!(
        pdf_has_filled_polygon(&hay),
        "ribbon must fill a polygon (h f), not only re rects; tail {}",
        &hay[hay.len().saturating_sub(400)..]
    );
    let rects = pdf_fill_boxes_in(&hay, 1.0, 0.0, 0.0);
    assert!(
...
#[test]
fn ribbon2_prst_fills_a_polygon_not_a_rect() {
    let body = preset_shape_body("ribbon2");
    let pdf = docx_to_pdf(&drawing_docx(&body)).expect("convert ribbon2");
    let hay = String::from_utf8_lossy(&pdf);
    assert!(
        pdf_has_filled_polygon(&hay),
        "ribbon2 must fill a polygon (h f), not only re rects; tail {}",
        &hay[hay.len().saturating_sub(400)..]
    );
    let rects = pdf_fill_boxes_in(&hay, 1.0, 0.0, 0.0);
    assert!(
...
#[test]
fn left_right_circular_arrow_prst_fills_a_polygon_not_a_rect() {
    let body = preset_shape_body("leftRightCircularArrow");
    let pdf = docx_to_pdf(&drawing_docx(&body)).expect("convert leftRightCircularArrow");
    let hay = String::from_utf8_lossy(&pdf);
    assert!(
        pdf_has_filled_polygon(&hay),
        "leftRightCircularArrow must fill a polygon (h f), not only re rects; tail {}",
        &hay[hay.len().saturating_sub(400)..]
    );
    let rects = pdf_fill_boxes_in(&hay, 1.0, 0.0, 0.0);
    assert!(
...
#[test]
fn wave_prst_fills_a_polygon_not_a_rect() {
    let body = preset_shape_body("wave");
    let pdf = docx_to_pdf(&drawing_docx(&body)).expect("convert wave");
    let hay = String::from_utf8_lossy(&pdf);
    assert!(
        pdf_has_filled_polygon(&hay),
        "wave must fill a polygon (h f), not only re rects; tail {}",
        &hay[hay.len().saturating_sub(400)..]
    );
    let rects = pdf_fill_boxes_in(&hay, 1.0, 0.0, 0.0);
    assert!(
...
#[test]
fn two_column_section_with_column_break_paints_side_by_side() {
    // xml leftover: sectPr w:cols (ECMA-376 17.6.4). num=2 space=720
    // (36pt) on letter + 72pt margins → col width 216, col2 x=324.
    // w:br type=column starts the next column on the same page.
    ...
    let pdf = docx_to_pdf(&minimal_docx_with_settings(body, "")).expect("convert 2-col");
    ...
    assert!(
        xs.iter().any(|x| (300.0..360.0).contains(x)),
        "column 2 starts at 324pt (72+216+36); xs={xs:?}"
    );
}
