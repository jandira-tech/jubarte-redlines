...
//! M412 — short color base × short threaded next: pure-I first next title.
//!
//! Word pure-I's "Text" then free-meshes "Text 2"×color base. Engine free-
//! meshed "Text" into base (lost pure-I title).
...
fn text_color_x_threaded_pure_i_text_title() {
    ...
    let pure_i_text = paras
        .iter()
        .any(|(i, d, t)| *i && !*d && t.trim() == "Text");
    assert!(
        pure_i_text,
        "expected pure-I title Text; got {:?}",
        paras
            .iter()
            .map(|(i, d, t)| format!(
                "{}{} {:?}",
