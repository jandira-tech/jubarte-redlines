...
fn m4_c4_para_mark_guards() {
    ...
    let res = do_lcs_algorithm(&mut d, cs, &s);
...
fn group(gt: ComparisonUnitGroupType, contents: Vec<ComparisonUnit>, sha: &str) -> ComparisonUnit {
    ComparisonUnit::Group(ComparisonUnitGroup {
        ...
        contents,
        level: 0,
        sha1: jubarte::comparer::atoms::Sha1Keyed::new(sha.to_string()),
...
fn m4_c8_step_h1_word_row() {
    ...
    let res = do_lcs_algorithm(&mut d, unk(vec![wa, row_l], vec![row_r]), &s);
...
fn m4_c9_step_h4_flatten() {
    ...
    let res = do_lcs_algorithm(&mut d, unk(vec![pg1], vec![pg2]), &s);
    assert_eq!(res.len(), 1);
    assert_eq!(res[0].correlation_status, CorrelationStatus::Unknown);
...
fn m4_c10_step_h5_row_cells() {
    ...
    let res = do_lcs_algorithm(&mut d, unk(vec![row_l], vec![row_r]), &s);
