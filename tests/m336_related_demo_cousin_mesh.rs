...
//! M336 — bullet_list_bold × bullet_list: Word free-meshes last item into bold
//! intro (DIMD MIX=1). Pure-I/D leaves Grapes separate (DID MIX=0).
...
fn bullet_list_bold_x_bullet_list_has_mix() {
    ...
    assert!(
        n_m >= 1,
        "Word free-meshes Grapes into bold intro; got MIX={n_m} seq={seq}"
    );
}
...
fn left_alignment_x_line_spacing_still_mmim() {
    ...
    assert_eq!(n_m, 3, "Word MIX=3 MMIM; got MIX={n_m}");
    assert_eq!(n_i, 1, "Word I=1; got I={n_i}");
}
